...
grant select, insert, update on public.portal_workspace_state to anon, authenticated;

drop policy if exists portal_workspace_state_read on public.portal_workspace_state;
create policy portal_workspace_state_read
on public.portal_workspace_state
for select
to anon, authenticated
using (true);

drop policy if exists portal_workspace_state_insert on public.portal_workspace_state;
create policy portal_workspace_state_insert
on public.portal_workspace_state
for insert
to anon, authenticated
with check (true);
...
create policy portal_workspace_state_update
on public.portal_workspace_state
for update
to anon, authenticated
using (true)
