alter table public.service_runs
  add column if not exists recheck_scope text not null default 'all_actionable'
  check (recheck_scope in ('all_actionable', 'failed', 'unverified', 'changed', 'full'));
...
comment on column public.service_runs.recheck_scope is
  'Exact maintenance intent shown in the UI: failed, unverified, changed, all actionable, or full refresh.';
