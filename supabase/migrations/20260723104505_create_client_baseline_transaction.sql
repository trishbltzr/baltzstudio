...
comment on function public.create_client_with_baseline(
  uuid, text, text, text, text, text, text, text, text, integer, integer
) is
  'Atomically creates or resolves one production client, its first real domain source, and one idempotent queued baseline Checkup run.';
