...
begin
  ...
  completed_state := p_state in ('ready', 'current', 'partial', 'blocked', 'failed', 'cancelled');
  ...
  set state = p_state,
      ...
      completed_at = case when completed_state then timezone('utc', now()) else null end,
      cancelled_at = case when p_state = 'cancelled' then timezone('utc', now()) else cancelled_at end,
      workflow_token_hash = case when completed_state then null else workflow_token_hash end,
      workflow_token_expires_at = case when completed_state then null else workflow_token_expires_at end,
...
comment on function public.workflow_transition_service_run(uuid, text, text, text, text, text, integer, integer, jsonb, text, text, text, text) is
  'Capability-scoped durable state transition. Completed and review-ready runs immediately revoke their dispatch capability.';

update public.service_runs
set workflow_token_hash = null,
    workflow_token_expires_at = null,
    updated_at = timezone('utc', now())
where state in ('ready', 'current', 'partial', 'blocked', 'failed', 'cancelled')
  and (workflow_token_hash is not null or workflow_token_expires_at is not null);
