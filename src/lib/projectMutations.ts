...
import { currentDashboardTimestamp } from "./dateDisplay";

export function updateTask(project: Project, taskId: string, status: TaskStatus): Project {
  const updatedAt = currentDashboardTimestamp();
  return {
    ...project,
    milestones: project.milestones.map(m => ({
      ...
      phases: m.phases.map(p => {
        ...
        let tasks = p.tasks.map(t => {
          if (t.id === taskId) return { ...t, status, updatedAt };
          if (status === "in_progress" && t.status === "in_progress") return { ...t, status: "not_started" as TaskStatus, updatedAt };
          return t;
        });
        ...
        if (status === "complete") {
          ...
          tasks = tasks.map((t, i) => {
            if (t.status === "in_progress") return { ...t, status: "not_started" as TaskStatus, updatedAt };
            if (i === nextNotStartedIndex) return { ...t, status: "in_progress" as TaskStatus, updatedAt };
            return t;
          });
          ...
          if (!hasActive && firstWaitingIndex !== -1) {
            tasks = tasks.map((t, i) => i === firstWaitingIndex ? { ...t, status: "in_progress" as TaskStatus, updatedAt } : t);
          }
        }
      ...
      if (index === targetIndex) {
        return {
          ...
          phases: milestone.phases.map(phase => ({
            ...phase,
            tasks: phase.tasks.map(task => ({ ...task, status: "complete" as TaskStatus, updatedAt })),
            gate: phase.gate ? { ...phase.gate, status: "approved" as GateStatus, approvedAt: phase.gate.approvedAt ?? updatedAt } : undefined,
          })),
        };
      }
...
export function sendGate(project: Project, gateId: string): Project {
  return { ...project, milestones: project.milestones.map(m => ({ ...m, phases: m.phases.map(p => ({ ...p, gate: p.gate?.id === gateId ? { ...p.gate, status: "sent" as GateStatus, sentAt: currentDashboardTimestamp() } : p.gate })) })) };
}

export function approveGate(project: Project, gateId: string): Project {
  return { ...project, milestones: project.milestones.map(m => ({ ...m, phases: m.phases.map(p => ({ ...p, gate: p.gate?.id === gateId ? { ...p.gate, status: "approved" as GateStatus, approvedAt: currentDashboardTimestamp() } : p.gate })) })) };
}
