...
export function FilterDropdown({ id, label, valueLabel, options, state, actions, icon }: {
  ...
  const btn = "display:inline-flex;align-items:center;gap:0.35rem;height:2.1rem;padding:0 0.6rem 0 0.85rem;border:1px solid " + (open ? "var(--accent-dim)" : "var(--border)") + ";border-radius:var(--radius-pill);background:var(--surface);color:var(--fg-muted);font-size:0.78rem;font-weight:500;cursor:pointer;white-space:nowrap";
  return (
      ...
      {open && (
        <>
          ...
          <div style={{ ...css("position:absolute;top:2.55rem;left:0;min-width:11.5rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);z-index:55;padding:0.3rem"), animation: "pt-ddin .15s ease" }}>
            {options.map(o => (
              <button key={o.label} onClick={() => { actions.closePop(); o.onClick(); }} style={css("display:flex;align-items:center;justify-content:space-between;gap:0.9rem;width:100%;text-align:left;padding:0.5rem 0.7rem;border:none;border-radius:calc(var(--radius) - 3px);background:" + (o.active ? "var(--accent-soft)" : "transparent") + ";color:" + (o.active ? "var(--accent)" : "var(--fg)") + ";font-size:0.79rem;font-weight:500;cursor:pointer")}>
