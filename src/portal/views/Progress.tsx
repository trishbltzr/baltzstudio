        ...
        <div style={css("min-width:0;flex:1;height:100%;display:flex;flex-direction:column;gap:0.3rem;position:relative;z-index:2;transition:flex-basis .28s cubic-bezier(.22,1,.36,1),opacity .2s ease")}>
          <div style={css("position:relative;z-index:4;display:flex;align-items:center;gap:0.42rem;height:2.05rem;border-radius:999px;background:var(--accent-grad);color:#fff;padding:0 0.4rem 0 0.5rem;box-sizing:border-box")}>
            ...
            <button
              ...
              style={css("height:1.6rem;border:0;border-radius:999px;background:" + (sent ? "rgba(216,246,224,.24)" : "rgba(255,255,255,.1)") + ";color:#fff;display:inline-flex;align-items:center;gap:0.35rem;cursor:" + (!hasUserMessage || sent ? "default" : "pointer") + ";padding:0 0.5rem;font-size:var(--text-xs);font-weight:500;opacity:" + (!hasUserMessage ? ".55" : "1") + ";white-space:nowrap;flex-shrink:0")}
            >
              <Icon name={sent ? "inbox" : "msg"} size={13} />
              {!state.isMobile && <span>{sent ? "Added to Inbox" : "Send as ticket"}</span>}
            </button>
            <button type="button" onClick={toggleHistory} aria-label={state.progressChatHistoryOpen ? "Hide chat history" : "View chat history"} aria-pressed={state.progressChatHistoryOpen} title={state.progressChatHistoryOpen ? "Hide chat history" : "View chat history"} className="pt-progress-chatbar-icon" style={css("width:1.6rem;height:1.6rem;border:0;border-radius:50%;background:" + (state.progressChatHistoryOpen ? "rgba(255,255,255,.16)" : "transparent") + ";color:#fff;display:grid;place-items:center;cursor:pointer;flex-shrink:0")}>
          ...
          {ticketFeedback && (
            <div className="pt-ticket-added-toast" style={css("position:absolute;right:0.75rem;top:2.45rem;z-index:6;display:inline-flex;align-items:center;gap:0.45rem;border:1px solid color-mix(in srgb,var(--success) 22%,var(--border-soft) 78%);border-radius:999px;background:color-mix(in srgb,var(--success) 14%,var(--surface) 86%);color:var(--success);padding:0.4rem 0.65rem;font-size:var(--text-2xs);font-weight:500;box-shadow:0 0.75rem 1.4rem color-mix(in srgb,var(--success) 12%,transparent 88%)")}>
...
function ProgressChatHistory({
  ...
  return (
    <aside className={"pt-progress-chat-history-sidebar" + (closing ? " is-exiting" : "")} style={css((state.isMobile ? "position:absolute;right:0.65rem;top:3rem;bottom:0.65rem;width:min(18rem,calc(100% - 1.3rem));z-index:8;" : "position:relative;width:17rem;height:100%;flex-shrink:0;") + "overflow:hidden;border:1px solid var(--border-soft);border-radius:1.15rem;background:color-mix(in srgb,var(--surface) 94%,white 6%);display:flex;flex-direction:column;box-shadow:0 1rem 2rem color-mix(in srgb,var(--fg) 7%,transparent 93%)")}>
        <div style={css("padding:0.75rem 0.8rem;border-bottom:1px solid var(--border-soft);display:flex;flex-direction:column;gap:0.6rem")}>
          <div style={css("display:flex;align-items:center;gap:0.65rem;min-width:0")}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={css("font-size:var(--text-md);font-weight:500;color:var(--fg)")}>Chat history</div>
              <div style={css("font-size:var(--text-2xs);color:var(--fg-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{sessions.length || "No"} saved session{sessions.length === 1 ? "" : "s"}</div>
            </div>
            <button type="button" onClick={onClose} aria-label="Close chat history" title="Close chat history" className="pt-iconbtn" style={css("width:1.85rem;height:1.85rem;border-radius:50%;border:1px solid var(--border-soft);background:var(--surface);color:var(--fg-muted);display:grid;place-items:center;cursor:pointer;flex-shrink:0")}><Icon name="x" size={12} /></button>
          </div>
          <div style={css("display:grid;grid-template-columns:" + (sessions.length > 0 ? "1fr 1fr" : "1fr") + ";gap:0.45rem")}>
            <button type="button" onClick={onNew} className="pt-softbtn" style={css("height:1.9rem;padding:0 0.7rem;border-radius:999px;border:1px solid var(--border-soft);background:var(--surface);color:var(--fg-muted);font-size:var(--text-xs);font-weight:500;cursor:pointer")}>New chat</button>
            {sessions.length > 0 && <button type="button" onClick={() => { if (window.confirm("Delete all Snapshot chat history? This cannot be undone.")) actions.clearProgressChatHistory(); }} className="pt-softbtn" style={css("height:1.9rem;padding:0 0.7rem;border-radius:999px;border:1px solid color-mix(in srgb,var(--danger) 25%,var(--border-soft) 75%);background:var(--surface);color:var(--danger);font-size:var(--text-xs);font-weight:500;cursor:pointer")}>Clear history</button>}
          </div>
        </div>
        <div className="pt-progress-chat-history-list" style={css("flex:1;min-height:0;overflow-y:auto;padding:0.45rem;display:flex;flex-direction:column;gap:0.3rem")}>
          {sessions.length === 0 && <div style={css("padding:1.5rem 0.8rem;text-align:center;color:var(--fg-muted);font-size:var(--text-sm)")}>Start a chat and it will appear here.</div>}
          {sessions.map(session => {
            const active = session.id === state.activeProgressChatId;
            return (
              <div key={session.id} className="pt-menuitem" style={css("display:flex;align-items:center;gap:0.35rem;width:100%;border:1px solid " + (active ? "var(--accent-dim)" : "transparent") + ";border-radius:0.85rem;background:" + (active ? "var(--accent-soft)" : "transparent") + ";padding:var(--space-1)")}>
                <button type="button" onClick={() => actions.selectProgressChatSession(session.id)} style={css("min-width:0;flex:1;display:flex;align-items:center;gap:0.65rem;border:0;border-radius:0.7rem;background:transparent;padding:0.37rem 0.4rem;cursor:pointer;text-align:left")}>
                  ...
                  <span style={css("min-width:0;flex:1")}>
                    <span style={css("display:block;font-size:var(--text-sm);font-weight:500;color:var(--fg);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{session.title}</span>
...
function ProgressChatPanel({
  ...
  const suggestions = state.role === "client"
    ? ["What needs me next?", "Show my latest checkup plan", "What changed recently?", "Show my open approvals"]
    : state.role === "dev"
      ? ["What needs attention today?", "Which client is blocked?", "Summarize my approvals", "What changed recently?"]
      : ["What should I fix first?", "Show checkup risks", "Which clients need a nudge?", "What changed recently?"];
  ...
  return (
    <section className={embedded ? "pt-progress-chat-panel" + (closing ? " is-exiting" : "") : undefined} style={css("height:" + (embedded ? "calc(100% - 2.35rem)" : (state.isMobile ? "29rem" : "34rem")) + ";border:" + (embedded ? "0" : "1px solid var(--border-soft)") + ";border-radius:" + (embedded ? "0.95rem" : "var(--radius-panel)") + ";background:" + (embedded ? "transparent" : "var(--surface)") + ";display:flex;flex-direction:column;overflow:hidden;text-align:left;position:relative")}>
      {!embedded && <div style={css("padding:0.95rem 1.05rem;border-bottom:1px solid var(--border-soft);display:flex;align-items:center;gap:0.7rem")}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <h3 style={css("margin:0;font-size:var(--text-lg);font-weight:500")}>Ask Baltz AI</h3>
          <p style={css("margin:0.12rem 0 0;font-size:var(--text-xs);color:var(--fg-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{hero.ph}</p>
        </div>
      </div>}
        ...
        <div style={css("min-height:100%;display:flex;flex-direction:column;justify-content:" + (messages.length === 0 ? "center" : "flex-end") + ";gap:0.7rem")}>
          {messages.length === 0 && (
            <div style={css("margin:0 auto;max-width:" + (state.isMobile ? "24rem" : "34rem") + ";text-align:center;padding:0.3rem 0")}>
              <h3 style={css("margin:0;font-size:var(--text-lg);font-weight:500;line-height:1.25")}>Start from the workspace, not a blank chat.</h3>
              <p style={css("margin:0.28rem auto 0;max-width:24rem;font-size:var(--text-xs);line-height:1.4;color:var(--fg-muted)")}>Ask about clients, checkups, funnels, tasks, or the next decision.</p>
              <div style={css("text-transform:uppercase;font-size:var(--text-label);font-weight:400;letter-spacing:0.04em;line-height:1.2;color:var(--fg-faint);text-align:center;margin-top:0.95rem;margin-bottom:0.42rem")}>Try asking</div>
      ...
      <div className={embedded ? "pt-progress-chat-composer" + (closing ? " is-exiting" : "") : undefined} style={css("border-top:1px solid color-mix(in srgb,var(--border-soft) 68%,transparent 32%);padding:0.52rem 0.55rem 0.48rem;background:transparent")}>
        ...
        <div style={css("display:flex;align-items:flex-end;gap:0.4rem;border:1px solid var(--border);border-radius:999px;background:color-mix(in srgb,var(--surface) 76%,transparent 24%);padding:0.34rem 0.4rem")}>
          ...
          <textarea
            ...
            rows={1}
            style={css("flex:1;min-height:2.2rem;max-height:7.5rem;overflow-y:auto;border:none;background:transparent;outline:none;resize:none;font-family:inherit;font-size:var(--text-md);line-height:1.45;color:var(--fg);padding:0.43rem 0;min-width:0")}
          />
...
export function Progress({ state, actions }: { state: PortalState; actions: PortalActions }) {
  ...
  return (
    <div style={css("display:flex;flex-direction:column;gap:var(--space-4)")}>
      <AttentionBand state={state} actions={actions} />
      ...
      <div className={chatOpen ? "pt-progress-below-fade" : undefined} style={css("display:flex;flex-direction:column;gap:var(--space-4)")}>
        <ProcessTracker state={state} actions={actions} />
        {progressBody}
      </div>
      <section style={css("display:flex;flex-direction:column;gap:0.65rem")}>
        <SectionHeader title="Calendar" sub="Task deadlines across your current workspace." actionLabel="Open board" onAction={() => { actions.patch({ taskView: "board" }); actions.setView("tasks"); }} />
