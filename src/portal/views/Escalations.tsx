  ...
  return (
    <div style={css("display:flex;flex-direction:column;gap:0.85rem")}>
      <div style={css("display:flex;align-items:center;gap:0.6rem;padding:0.85rem 1.1rem;border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:oklch(0.985 0.012 22);box-shadow:inset 0 0 0 1px oklch(0.88 0.04 20 / 0.25)")}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}><div style={css("font-weight:500;font-size:var(--text-md)")}>You&apos;re the final tier</div><div style={css("font-size:var(--text-xs);color:var(--fg-muted)")}>Development escalates plan, billing, access and out-of-scope calls to you. Nothing routes higher.</div></div>
      ...
      <div style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface-alt);padding:0.9rem;display:flex;flex-direction:column;gap:0.6rem")}>
        ...
        {open.length === 0 && <div style={css("padding:2.5rem 1rem;text-align:center;border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);color:var(--fg-faint);font-size:var(--text-base)")}>Nothing needs your decision right now.</div>}
        <div style={css("display:grid;grid-template-columns:repeat(auto-fill,minmax(17rem,1fr));gap:0.6rem")}>
          {open.map(e => (
            <div key={e.id} style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);padding:1.05rem 1.15rem")}>
              ...
              <button onClick={() => actions.resolveEscalation(e.id)} className="pt-op" style={css("display:inline-flex;align-items:center;gap:0.35rem;height:2rem;padding:0 0.95rem;border-radius:var(--radius-pill);border:none;background:var(--accent);color:#fff;font-weight:500;font-size:var(--text-xs);cursor:pointer")}><Icon name="checkmark" size={15} /> Resolve</button>
            </div>
          ))}
        </div>
      </div>
      ...
      {done.length > 0 && (
        <>
          <div style={css("font-size:var(--text-2xs);letter-spacing:0.02em;color:var(--fg-faint);font-weight:500;padding:0.5rem 0.15rem 0.1rem")}>Resolved</div>
          {done.map(e => (
            <div key={e.id} style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface-alt);padding:0.8rem 1.15rem;display:flex;align-items:center;gap:0.6rem")}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}><div style={css("font-size:var(--text-base);color:var(--fg-muted);text-decoration:line-through")}>{e.title}</div><div style={css("font-size:var(--text-xs);color:var(--fg-faint)")}>{clientLine(e)}</div></div>
              <button onClick={() => reopen(e.id)} className="pt-iconbtn" style={css("font-size:var(--text-2xs);font-weight:500;color:var(--fg-muted);background:transparent;border:1px solid var(--border);border-radius:var(--radius-pill);padding:0.3rem 0.75rem;cursor:pointer;flex-shrink:0")}>Reopen</button>
            </div>
          ))}
        </>
      )}
