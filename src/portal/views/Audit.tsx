  ...
  if (!client) {
    return (
      <GuidedIntakeSelector
        ...
        controls={
          <>
            <span style={css("display:inline-flex;align-items:center;gap:0.35rem;padding:0.45rem 0.75rem;border:1px solid var(--border);border-radius:999px;background:var(--surface-alt);font-size:var(--text-2xs);color:var(--fg-muted)")}><span style={css("width:0.42rem;height:0.42rem;border-radius:50%;background:var(--success)")} />{auditReadyCount} completed</span>
            <span style={css("display:inline-flex;align-items:center;gap:0.35rem;padding:0.45rem 0.75rem;border:1px solid var(--border);border-radius:999px;background:var(--surface-alt);font-size:var(--text-2xs);color:var(--fg-muted)")}><span style={css("width:0.42rem;height:0.42rem;border-radius:50%;background:var(--warn)")} />{pendingAuditCount} still in intake</span>
            <div style={{ position: "relative" }}>
              <button type="button" onClick={() => setLaunchOpen(v => !v)} style={css("display:inline-flex;align-items:center;gap:0.42rem;min-height:2.3rem;padding:0 0.95rem;border:none;border-radius:999px;background:var(--cocoon);color:#fff;font-size:var(--text-xs);font-weight:500;cursor:pointer")}><Icon name="audit" size={15} />Start checkup</button>
              {launchOpen && (
                <>
                  ...
                  <div style={{ ...css("position:absolute;top:2.7rem;right:0;width:min(24rem,calc(100vw - 2rem));padding:0.45rem;border:1px solid var(--border);border-radius:1rem;background:color-mix(in srgb,var(--surface) 94%,white 6%);z-index:35"), animation: "pt-ddin .16s ease" }}>
                    <div style={css("padding:0.5rem 0.65rem 0.45rem")}>
                      <div style={css("font-size:var(--text-sm);font-weight:500;color:var(--fg)")}>Choose a client</div>
                      <div style={css("font-size:var(--text-2xs);color:var(--fg-faint);margin-top:0.18rem")}>Generating an audit always opens the intake flow first.</div>
                    </div>
                      ...
                      {STUDIO_CLIENTS.map(c => (
                        ...
                        >
                          ...
                          <span style={{ flex: 1, minWidth: 0 }}>
                            <span style={css("display:block;font-size:var(--text-sm);font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{c.name}</span>
                            <span style={css("display:block;font-size:var(--text-2xs);color:var(--fg-faint);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{c.audited ? "Audit complete — start a fresh intake" : "Needs intake — start audit"}</span>
                          </span>
                          <span style={css("display:inline-flex;align-items:center;font-size:var(--text-2xs);font-weight:500;padding:0.16rem 0.5rem;border-radius:999px;background:var(--cocoon-soft, color-mix(in srgb,var(--cocoon) 16%,white 84%));color:var(--cocoon)")}>Start</span>
                        </button>
                      ))}
                    </div>
                  </div>
        ...
        cards={STUDIO_CLIENTS.map(c => ({
          ...
          primaryLabel: c.audited ? "View report" : "Start checkup", onPrimary: () => selectClient(c.id),
