...
const AUDIT_STAGES: Array<{ id: string; label: string; sections: SeoSection[] }> = [
  { id: "crawl", label: "Crawl & inventory", sections: ["sources"] },
  { id: "audit", label: "Audit findings", sections: ["overview"] },
  { id: "search", label: "Keywords & pages", sections: ["keywords"] },
  { id: "report", label: "Report & priorities", sections: ["audit-report"] },
  { id: "plan", label: "Action plan", sections: ["metadata", "architecture", "roadmap"] },
];
...
function Pill({ children, tone = "muted" }: { children: ReactNode; tone?: "muted" | "accent" | "success" | "warn" | "danger" }) {
  ...
  return <span style={css("display:inline-flex;align-items:center;gap:.3rem;padding:.2rem .5rem;border-radius:999px;background:color-mix(in srgb," + color + " 10%,var(--surface-alt) 90%);color:" + color + ";font-size:.66rem;font-weight:500;white-space:nowrap")}>{children}</span>;
}
...
function SeoWorkspace({ state, actions }: { state: PortalState; actions: PortalActions }) {
  ...
  const cards = useMemo(() => availableClients.filter(client => savedProjects[client.id]?.rows?.length).map(client => {
    ...
    return {
      id: `seo-${client.id}`,
      name: client.name,
      subtitle: scored ? `${normalizeSourceType(project.sourceType as string)} · ${new Date(project.importedAt).toLocaleDateString()}` : "Crawl-led SEO review",
  ...
  return <div style={css("width:100%;padding:" + (state.isMobile ? "1rem .9rem 1.5rem" : "1.4rem 1.5rem"))}>
    <div style={css("width:100%;max-width:60rem;margin:0 auto;display:flex;flex-direction:column;gap:.85rem;box-sizing:border-box") }>
      ...
      <aside style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);overflow:hidden" + (state.isMobile ? "" : ";position:sticky;top:.5rem")) }>
        ...
        <nav aria-label="SEO project sections" style={css("padding:.35rem 0") }>
          {stages.map((stage, index) => {
            ...
            const disabled = !rows.length && stage.id !== "crawl";
            ...
            const stageLabel = clientReport && stage.id === "report" ? "Your report" : stage.label;
            const destination = clientReport && stage.id === "report" ? "audit-report" : stage.sections[0];
            return <button key={stage.id} type="button" disabled={disabled} onClick={() => setActiveSection(destination)} style={css("width:calc(100% - 1rem);margin:0 .5rem;display:flex;align-items:center;gap:.65rem;min-height:2.35rem;padding:.3rem .6rem;border:none;border-radius:999px;text-align:left;cursor:" + (disabled ? "default" : "pointer") + ";background:" + (active ? "color-mix(in srgb,var(--success) 9%,white 91%)" : "transparent") + ";opacity:" + (disabled ? ".58" : "1"))}><span style={css("width:1.2rem;height:1.2rem;border-radius:50%;display:grid;place-items:center;flex-shrink:0;font-size:.62rem;font-weight:500;" + dot)}>{done ? <Icon name="checkmark" size={9}/> : index + 1}</span><span style={css("min-width:0;font-size:var(--text-base);font-weight:" + (active || done ? "500" : "400") + ";color:" + (active ? "var(--success)" : disabled ? "var(--fg-muted)" : "var(--fg)") + ";white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.2")}>{stageLabel}</span></button>;
          })}
        </nav>
      ...
      <main style={css("min-width:0;flex:1;display:flex;flex-direction:column;gap:.85rem") }>
        ...
        {activeSection === "roadmap" && <RoadmapView showToast={actions.showToast} rows={rows} mobile={state.isMobile} />}
      </main>
    </div>
...
function WorkspaceHeader({ section, client, mobile }: { section: SeoSection; client: StudioClient; mobile: boolean }) {
  ...
  return <Panel style="padding:1rem"><div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;flex-wrap:wrap") }><div><span style={css("display:block;text-transform:uppercase;letter-spacing:.04em;font-size:.64rem;color:var(--accent);margin-bottom:.3rem")}>{client.name} · SEO · AIO · GEO</span><h1 style={css("margin:0;font-size:" + (mobile ? "1.08rem" : "1.2rem") + ";font-weight:500;line-height:1.2")}>{labels[section][0]}</h1><p style={css("margin:.35rem 0 0;color:var(--fg-muted);font-size:.76rem;line-height:1.5;max-width:34rem")}>{labels[section][1]}</p></div></div></Panel>;
}
...
function SourcesView({ sourceType, sourceName, rows, importedAt, dragging, sitemapUrl, sitemapError, sitemapLoading, onSitemapUrl, onSitemap, onSource, onBrowse, onDrop, onDrag, onContinue }: SourcesViewProps) {
  ...
  return <div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(18rem,1fr));gap:.85rem") }>
    ...
    <Panel style="padding:1rem">
      <h2 style={css("margin:0;font-size:.95rem;font-weight:500")}>2. {sourceType === "Sitemap crawl" ? "Crawl the live sitemap" : "Import the CSV"}</h2>
...
function MetricMini({ value, label }: { value: number; label: string }) { return <div style={css("padding:.65rem;border:1px solid var(--border-soft);border-radius:.7rem;background:var(--surface-alt)") }><div style={css("font-size:1.05rem;font-weight:500")}>{value}</div><div style={css("font-size:.61rem;color:var(--fg-faint);margin-top:.15rem")}>{label}</div></div>; }
...
function LegacyOverviewView({ rows, stats, onGo }: { rows: CrawlRow[]; stats: SeoStats; onGo: (section: SeoSection) => void }) {
  ...
  return <><div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(9rem,1fr));gap:.65rem") }><Kpi label="Crawl health" value={`${stats.health}%`} tone={stats.health > 80 ? "success" : "warn"}/><Kpi label="Active pages" value={String(stats.active.length)}/><Kpi label="Indexable" value={String(stats.indexable.length)} tone="success"/><Kpi label="Issues found" value={String(issues)} tone={issues ? "warn" : "success"}/></div><div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(19rem,1fr));gap:.85rem") }><Panel style="padding:1rem"><SectionTitle title="Crawl health" sub="Status and indexability across the imported inventory"/><div style={css("display:flex;align-items:center;gap:1rem;margin-top:1rem;flex-wrap:wrap") }><div style={css("width:7rem;height:7rem;border-radius:50%;background:conic-gradient(var(--success) 0 " + stats.health + "%,var(--warn) " + stats.health + "% " + Math.min(100, stats.health + 8) + "%,var(--danger) " + Math.min(100, stats.health + 8) + "%);display:grid;place-items:center") }><div style={css("width:5.6rem;height:5.6rem;border-radius:50%;background:var(--surface);display:grid;place-items:center;text-align:center") }><div><strong style={css("display:block;font-size:1.35rem;font-weight:500")}>{stats.health}</strong><span style={css("font-size:.58rem;color:var(--fg-faint)")}>health score</span></div></div></div><div style={css("flex:1;min-width:10rem;display:flex;flex-direction:column;gap:.5rem") }><Bar label="Healthy 2xx" value={stats.active.length} total={rows.length} color="var(--success)"/><Bar label="Redirects" value={stats.redirects.length} total={rows.length} color="var(--warn)"/><Bar label="Broken" value={stats.broken.length} total={rows.length} color="var(--danger)"/></div></div></Panel><Panel style="padding:1rem"><SectionTitle title="Priority issues" sub="What needs attention before growth work" action="Review all" onAction={() => onGo("issues")}/><div style={css("display:flex;flex-direction:column;gap:.45rem;margin-top:.85rem") }><IssueLine label="Broken pages" count={stats.broken.length} tone="danger"/><IssueLine label="Redirecting URLs" count={stats.redirects.length} tone="warn"/><IssueLine label="Missing meta descriptions" count={stats.missingDescription.length} tone="warn"/><IssueLine label="Missing page titles" count={stats.missingTitle.length} tone="danger"/></div></Panel></div><Panel style="padding:1rem"><SectionTitle title="Audit evidence" sub="Review the complete inventory, findings, and readiness gate before finalizing priorities."/><div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(9rem,1fr));gap:.55rem;margin-top:.8rem") }>{[{ icon:"file",label:"Inventory",value:`${rows.length} URLs`,go:"inventory"},{icon:"alert",label:"Findings",value:`${issues} issues`,go:"issues"},{icon:"check",label:"Readiness",value:`${SEO_READINESS_ITEMS.length} checks`,go:"readiness"},{icon:"chart",label:"Audit report",value:"Prioritized summary",go:"audit-report"}].map(item => <button key={item.label} type="button" onClick={() => onGo(item.go as SeoSection)} style={css("padding:.75rem;border:1px solid var(--border-soft);border-radius:.8rem;background:var(--surface-alt);text-align:left;cursor:pointer;color:var(--fg)") }><Icon name={item.icon} size={15}/><span style={css("display:block;margin-top:.5rem;font-size:.72rem;font-weight:500")}>{item.label}</span><span style={css("display:block;margin-top:.15rem;font-size:.61rem;color:var(--fg-faint)")}>{item.value}</span></button>)}</div></Panel></>;
}
...
function ReadinessSummaryCard({ readiness, ai }: { readiness: Record<string, SeoReadinessStatus>; ai: Record<string, ReadinessAiResult> }) {
  ...
  return <Panel style="overflow:hidden">
    <div style={css("padding:1rem 1.1rem;border-bottom:1px solid var(--border-soft)") }>
      <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem;flex-wrap:wrap") }><SectionTitle title="SEO audit checklist" sub="Technical, on-page, architecture, AIO/GEO, and measurement checks are evaluated from the available evidence."/><button type="button" aria-expanded={open} onClick={() => setOpen(value => !value)} className="pt-softbtn" style={css("flex-shrink:0;min-height:2rem;padding:0 .75rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);color:var(--fg-muted);font-size:.64rem;font-weight:500;cursor:pointer")}>{open ? "Hide checklist" : "Show checklist"}</button></div>
      <div style={css("display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.7rem") }>{counts.map(item => <span key={item.label} style={css("font-size:.62rem;font-weight:500;border:1px solid var(--border-soft);border-radius:999px;background:var(--surface-alt);color:var(--fg-muted);padding:.18rem .48rem")}>{item.count} {item.label.toLowerCase()}</span>)}</div>
    </div>
    {open && <div style={css("display:flex;flex-direction:column;gap:.65rem;padding:.65rem") }>{SEO_READINESS_GROUPS.map(group => { const categoryItems = items.filter(item => item.group === group.title); return <section key={group.title} style={css("overflow:hidden;border:1px solid var(--border-soft);border-radius:.78rem;background:var(--surface)") }><div style={css("padding:.68rem .75rem;border-bottom:1px solid var(--border-soft);background:var(--surface-alt)") }><h3 style={css("margin:0;font-size:.72rem;font-weight:500")}>{group.title}</h3><p style={css("margin:.18rem 0 0;font-size:.58rem;line-height:1.4;color:var(--fg-faint)")}>{group.description}</p></div><div style={css("display:flex;flex-direction:column") }>{categoryItems.map(item => <div key={item.id} style={css("display:grid;grid-template-columns:1.45rem minmax(0,1fr) auto;gap:.55rem;align-items:start;padding:.7rem .75rem;border-bottom:1px " + (item.meta.dashed ? "dashed" : "solid") + " var(--border-soft);background:var(--surface)") }><span aria-hidden="true" style={css("width:1.35rem;height:1.35rem;border-radius:50%;display:grid;place-items:center;background:" + item.meta.bg + ";color:" + item.meta.color + ";font-size:.72rem;font-weight:500")}>{item.meta.symbol}</span><div style={css("min-width:0") }><strong style={css("display:block;font-size:.68rem;font-weight:500;line-height:1.35")}>{item.label}</strong><span style={css("display:block;margin-top:.18rem;font-size:.6rem;line-height:1.4;color:var(--fg-muted)")}>{item.evidence || item.detail}</span></div><span style={css("font-size:.56rem;font-weight:500;white-space:nowrap;color:" + item.meta.color + ";background:" + item.meta.bg + ";border-radius:999px;padding:.15rem .4rem")}>{item.meta.label}</span></div>)}</div></section>; })}</div>}
  </Panel>;
}
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.85rem") }>
    <Panel style="padding:1.05rem 1.15rem;background:linear-gradient(135deg,var(--surface),color-mix(in srgb,var(--success-soft) 50%,var(--surface) 50%))">
      <div style={css("display:flex;gap:1.1rem;align-items:center;flex-wrap:wrap") }>
        <div aria-label={`SEO health score ${stats.health} out of 100`} style={css("width:auto;min-width:6.6rem;flex-shrink:0;display:flex;flex-direction:column;gap:.42rem;padding:.85rem .95rem;border-radius:.9rem;background:color-mix(in srgb," + healthColor + " 10%,var(--surface) 90%)") }><div style={css("display:flex;align-items:baseline;gap:.25rem") }><strong style={css("font-size:2.1rem;font-weight:500;line-height:.85;color:" + healthColor + ";font-variant-numeric:tabular-nums")}>{stats.health}</strong><span style={css("font-size:.6rem;color:var(--fg-faint)")}>/100</span></div><div style={css("height:.4rem;border-radius:999px;background:color-mix(in srgb," + healthColor + " 20%,var(--surface) 80%)") }><div style={css("height:100%;border-radius:999px;width:" + Math.max(2, stats.health) + "%;background:" + healthColor)}/></div><span style={css("font-size:.56rem;color:var(--fg-faint)")}>SEO health</span></div>
        <div style={css("flex:1;min-width:min(13rem,100%)") }><Pill tone={stats.health >= 75 ? "success" : "warn"}>{scoreLabel}</Pill><h2 style={css("margin:.6rem 0 0;font-size:1.1rem;font-weight:500;line-height:1.22")}>{stats.health >= 75 ? "The crawl foundation is usable." : "Fix the crawl foundation first."}</h2><p style={css("margin:.3rem 0 0;font-size:.68rem;line-height:1.5;color:var(--fg-muted)")}>{issueTotal ? `${issueTotal} signals need review across ${rows.length} crawled URLs.` : `No priority crawl issues found across ${rows.length} URLs.`}</p></div>
      </div>
    ...
    <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "minmax(0,1fr) minmax(0,1fr)") + ";gap:.8rem;align-items:stretch") }>
      <Panel style="padding:1rem 1.1rem;height:100%;box-sizing:border-box">
        ...
        <div style={css("display:flex;align-items:baseline;gap:.4rem;margin-top:.75rem") }><strong style={css("font-size:1.6rem;font-weight:500;line-height:.9;font-variant-numeric:tabular-nums")}>{rows.length}</strong><span style={css("font-size:.6rem;color:var(--fg-faint)")}>total URLs</span></div>
        ...
        <div style={css("display:flex;flex-direction:column;gap:.42rem;margin-top:.75rem") }>{([["Healthy 2xx", stats.active.length, "var(--success)"], ["Redirects", stats.redirects.length, "var(--warn)"], ["Broken 4xx", stats.broken.length, "var(--danger)"]] as const).map(([label, value, color]) => <div key={label} style={css("display:flex;align-items:center;justify-content:space-between;gap:.6rem") }><span style={css("display:flex;align-items:center;gap:.4rem;font-size:.66rem;color:var(--fg-muted)") }><span style={css("width:.5rem;height:.5rem;border-radius:50%;background:" + color)}/>{label}</span><span style={css("font-size:.66rem;font-variant-numeric:tabular-nums") }><strong style={css("font-weight:500")}>{value}</strong><span style={css("color:var(--fg-faint)")}> · {rows.length ? Math.round(value / rows.length * 100) : 0}%</span></span></div>)}</div>
      </Panel>
      <Panel style="padding:1rem 1.1rem;height:100%;box-sizing:border-box">
        ...
        <div style={css("display:grid;grid-template-columns:repeat(" + depthGroups.length + ",minmax(2rem,1fr));gap:.45rem;height:8rem;align-items:end;margin-top:.85rem;padding:.55rem .45rem 0;border-left:1px solid var(--border-soft);border-bottom:1px solid var(--border-soft);background:linear-gradient(0deg,var(--surface-alt),transparent)") }>{depthGroups.map((item, index) => { const pages = rows.filter(row => row.depth === item.depth); return <DataVizDatum key={item.depth} label={`Depth ${item.depth}`} value={`${item.count} page${item.count === 1 ? "" : "s"}`} lines={pages.map(row => row.url)} align={index > depthGroups.length / 2 ? "right" : "left"} style="height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;gap:.3rem;min-width:0"><span style={css("font-size:.6rem;color:var(--fg-muted)")}>{item.count}</span><span style={css("display:block;width:min(72%,3rem);height:" + Math.max(item.count ? 12 : 2, item.count / maxDepthCount * 82) + "%;min-height:2px;border-radius:.48rem .48rem 0 0;background:" + (item.depth <= 2 ? "var(--success)" : item.depth === 3 ? "var(--warn)" : "var(--danger)"))}/><span style={css("font-size:.56rem;color:var(--fg-faint);white-space:nowrap")}>Depth {item.depth}</span></DataVizDatum>; })}</div>
      </Panel>
    </div>
...
function Kpi({ label, value, tone = "default" }: { label: string; value: string; tone?: "default" | "success" | "warn" }) { const color = tone === "success" ? "var(--success)" : tone === "warn" ? "var(--warn)" : "var(--fg)"; return <Panel style="padding:.85rem"><div style={css("font-size:.63rem;color:var(--fg-faint)")}>{label}</div><div style={css("font-size:1.35rem;font-weight:500;margin-top:.3rem;color:" + color) }>{value}</div></Panel>; }
function SectionTitle({ title, sub, action, onAction }: { title: string; sub: string; action?: string; onAction?: () => void }) { return <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem") }><div><h2 style={css("margin:0;font-size:.88rem;font-weight:500")}>{title}</h2><p style={css("margin:.22rem 0 0;font-size:.65rem;color:var(--fg-faint);line-height:1.4")}>{sub}</p></div>{action && <button type="button" onClick={onAction} style={css("border:none;background:none;color:var(--accent);font-size:.65rem;cursor:pointer;white-space:nowrap")}>{action} →</button>}</div>; }
function Bar({ label, value, total, color }: { label: string; value: number; total: number; color: string }) { const pct = total ? Math.max(value ? 4 : 0, Math.round(value / total * 100)) : 0; return <div><div style={css("display:flex;justify-content:space-between;font-size:.62rem;color:var(--fg-muted);margin-bottom:.25rem") }><span>{label}</span><span>{value}</span></div><div style={css("height:.38rem;border-radius:999px;background:var(--surface-alt)") }>{value > 0 && <DataVizDatum label={label} value={`${value} of ${total} · ${pct}%`} style={"display:block;height:100%;width:" + pct + "%;background:" + color + ";border-radius:999px"}/>}</div></div>; }
function IssueLine({ label, count, tone }: { label: string; count: number; tone: "danger" | "warn" }) { return <div style={css("display:flex;align-items:center;justify-content:space-between;gap:.6rem;padding:.6rem .7rem;border:1px solid var(--border-soft);border-radius:.7rem;background:var(--surface-alt)") }><span style={css("font-size:.7rem")}>{label}</span><Pill tone={count ? tone : "success"}>{count || "Clear"}</Pill></div>; }
...
function InventoryView({ rows, allRows, query, onQuery }: { rows: CrawlRow[]; allRows: CrawlRow[]; query: string; onQuery: (value: string) => void }) {
  ...
  return <Panel style="position:relative">
    <div style={css("padding:.85rem 1rem;border-bottom:1px solid var(--border-soft);display:flex;align-items:center;justify-content:space-between;gap:.7rem;flex-wrap:wrap") }>
      ...
      <div style={css("display:flex;align-items:center;gap:.35rem;flex-wrap:nowrap") }><label style={css("height:2rem;display:flex;align-items:center;gap:.4rem;padding:0 .65rem;border:1px solid var(--border);border-radius:999px;background:var(--surface-alt);width:13rem;max-width:calc(100vw - 8rem)") }><Icon name="search" size={13}/><input value={query} onChange={event => onQuery(event.target.value)} placeholder="Search pages" style={css("border:none;outline:none;background:transparent;width:100%;font-size:.68rem;color:var(--fg)")}/></label><div style={css("position:relative;flex-shrink:0") }><button type="button" aria-label={`Choose columns · ${visibleColumns.length} visible`} title="Choose columns" aria-expanded={columnsOpen} aria-haspopup="dialog" onClick={() => setColumnsOpen(value => !value)} className="pt-softbtn" style={css("width:2rem;height:2rem;display:grid;place-items:center;padding:0;border:1px solid var(--border);border-radius:50%;background:var(--surface);color:var(--fg-muted);cursor:pointer")}><Icon name="funnel" size={13}/></button>{columnsOpen && <div role="dialog" aria-label="Choose crawled page columns" style={css("position:absolute;right:0;top:calc(100% + .4rem);z-index:20;width:min(18rem,82vw);border:1px solid var(--border);border-radius:.82rem;background:var(--surface);box-shadow:0 14px 32px rgb(50 30 30 / .16);overflow:hidden") }><div style={css("display:flex;align-items:center;justify-content:space-between;gap:.5rem;padding:.65rem .7rem;border-bottom:1px solid var(--border-soft)") }><div><strong style={css("display:block;font-size:.7rem;font-weight:500")}>Visible columns</strong><span style={css("display:block;margin-top:.12rem;font-size:.56rem;color:var(--fg-faint)")}>Choose at least one field.</span></div><div style={css("display:flex;gap:.25rem") }><button type="button" onClick={setEssential} style={css("border:none;background:none;color:var(--accent);font-size:.58rem;cursor:pointer")}>Essential</button><button type="button" onClick={() => setSelectedColumns(evidenceColumns)} style={css("border:none;background:none;color:var(--accent);font-size:.58rem;cursor:pointer")}>All</button></div></div><div style={css("display:flex;flex-direction:column;max-height:17rem;overflow:auto;padding:.35rem") }>{evidenceColumns.map(column => <label key={column} style={css("display:flex;align-items:center;gap:.5rem;min-height:2rem;padding:.35rem .45rem;border-radius:.55rem;cursor:pointer;font-size:.63rem;color:var(--fg-muted)") }><input type="checkbox" checked={selectedColumns.includes(column)} onChange={() => toggleColumn(column)} style={css("accent-color:var(--accent)")}/><span style={css("min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap")}>{labelFor(column)}</span></label>)}</div></div>}</div></div>
    </div>
...
function IssuesView({ rows, stats, ai, onRoadmap }: { rows: CrawlRow[]; stats: SeoStats; ai: AiVisibilityStats; onRoadmap: () => void }) {
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.8rem") }>
    <Panel style="padding:1.05rem 1.15rem;background:linear-gradient(135deg,var(--surface),color-mix(in srgb,var(--warn-soft) 38%,var(--surface) 62%))">
      <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:1.2rem;flex-wrap:wrap") }>
        <div style={{ maxWidth: "32rem", minWidth: 0 }}>
          <span style={css("font-size:.58rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--warn)")}>SEO issues</span>
          <h2 style={css("margin:.5rem 0 0;font-size:1.15rem;font-weight:500;line-height:1.2")}>{total ? `${total} signals across ${openCount} finding type${openCount === 1 ? "" : "s"}` : "No priority issues found"}</h2>
          <p style={css("margin:.3rem 0 0;font-size:.7rem;line-height:1.55;color:var(--fg-muted)")}>{total ? "Ranked by volume — the critical signals block crawling, indexing, and the click, so fix those first." : "The crawl surfaced no broken pages, missing signals, or thin content worth flagging."}</p>
        </div>
        <div style={css("display:flex;gap:.5rem;flex-wrap:wrap") }>
          {[["Critical", criticalCount, "var(--danger)"], ["Medium", mediumCount, "var(--warn)"], ["Opportunity", oppCount, "var(--accent)"]].map(([label, value, color]) => <div key={String(label)} style={css("min-width:5.2rem;padding:.6rem .7rem;border:1px solid var(--border-soft);border-radius:.75rem;background:var(--surface)") }><strong style={css("font-size:1.5rem;font-weight:500;line-height:.9;color:" + (Number(value) ? String(color) : "var(--fg-faint)") + ";font-variant-numeric:tabular-nums")}>{value}</strong><span style={css("display:block;margin-top:.25rem;font-size:.56rem;color:var(--fg-faint)")}>{label}</span></div>)}
        </div>
      </div>
    ...
    <Panel style="padding:1rem 1.1rem">
      ...
      <div style={css("display:flex;flex-direction:column;gap:.5rem;margin-top:.85rem") }>{ranked.map(item => <div key={item.title} style={css("display:grid;grid-template-columns:minmax(0,1fr) minmax(5rem,9rem) auto;gap:.7rem;align-items:center;padding:.62rem .72rem;border:1px solid " + (item.count ? "color-mix(in srgb," + toneColor[item.tone] + " 26%,var(--border-soft) 74%)" : "var(--border-soft)") + ";border-radius:.72rem;background:var(--surface)") }>
        <div style={css("min-width:0") }><div style={css("font-size:.73rem;font-weight:500;line-height:1.2")}>{item.title}</div><div style={css("font-size:.6rem;color:var(--fg-faint);line-height:1.35;margin-top:.12rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{item.why}</div></div>
        ...
        <div style={css("display:flex;align-items:center;justify-content:flex-end;gap:.35rem;min-width:2.8rem") }><strong style={css("font-size:.82rem;font-weight:500;text-align:right;color:" + (item.count ? "var(--fg)" : "var(--fg-faint)") + ";font-variant-numeric:tabular-nums")}>{item.count}</strong><span title={item.count ? item.severity : "Clear"} aria-label={item.count ? `${item.severity} finding` : "Clear finding"} style={css("display:inline-flex;color:" + (item.count ? toneColor[item.tone] : "var(--success)"))}><Icon name="flag" size={12}/></span></div>
      </div>)}</div>
    </Panel>
  </div>;
}
    ...
    <Panel style="padding:1.05rem 1.15rem;background:linear-gradient(135deg,var(--surface),color-mix(in srgb,var(--accent-soft) 42%,var(--surface) 58%))">
      <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:1.2rem;flex-wrap:wrap") }>
        <div style={{ maxWidth: "34rem", minWidth: 0 }}>
          <span style={css("display:inline-flex;align-items:center;gap:.35rem;font-size:.58rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--accent)") }><Icon name="sparkle" size={12}/>Evidence-based SEO audit</span>
          <h2 style={css("margin:.5rem 0 0;font-size:1.15rem;font-weight:500;line-height:1.2")}>{aiCount} of {total} checks cleared automatically</h2>
      ...
      <div style={css("display:flex;gap:.9rem;flex-wrap:wrap;margin-top:.55rem;font-size:.58rem;color:var(--fg-muted)") }>
    ...
    {SEO_READINESS_GROUPS.map(group => {
      ...
      return <section key={group.title} style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);overflow:hidden") }>
        <div style={css("padding:.85rem 1.1rem;border-bottom:1px solid var(--border-soft)") }>
          <h3 style={css("margin:0;font-size:.9rem;font-weight:500")}>{group.title}</h3>
          <p style={css("margin:.18rem 0 0;font-size:.6rem;line-height:1.4;color:var(--fg-faint);max-width:40rem")}>{group.description}</p>
          <div style={css("display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.6rem") }>{counts.filter(([count]) => count > 0).map(([count, label, tone, bg]) => <span key={label} style={css("font-size:.62rem;font-weight:500;color:" + tone + ";background:" + bg + ";border-radius:999px;padding:.16rem .5rem")}>{count} {label}</span>)}</div>
        </div>
        <div style={css("display:flex;flex-direction:column") }>{items.map(item => { const t = readinessTone(item.status); const actionable = item.status !== "confirmed"; return <div key={item.id} style={css("display:grid;grid-template-columns:1.55rem minmax(0,1fr) auto;gap:.6rem;align-items:start;padding:.7rem 1rem;border-bottom:1px solid var(--border-soft)") }>
          <span style={css("width:1.45rem;height:1.45rem;border-radius:50%;display:grid;place-items:center;background:" + t.bg + ";color:" + t.color + ";font-size:.75rem;font-weight:500")}>{t.icon}</span>
          <div style={css("min-width:0") }>
            <div style={css("font-size:.78rem;font-weight:500;line-height:1.4")}>{item.label}</div>
            <div style={css("font-size:.68rem;color:var(--fg-muted);line-height:1.45;margin-top:.2rem")}>{item.evidence || item.detail}</div>
            {actionable && <div style={css("display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;margin-top:.5rem") }><div style={css("display:flex;gap:.28rem;flex-wrap:wrap") }>{(["confirmed", "warning", "blocked"] as SeoReadinessStatus[]).map(s => <ReadinessStatusButton key={s} status={s} selected={item.status === s} onClick={() => onChange(item.id, s)}/>)}</div>{!item.status && <button type="button" onClick={() => onRequestAccess(item.label)} style={css("display:inline-flex;align-items:center;gap:.28rem;border:none;background:none;padding:0;color:var(--accent);font-size:.6rem;font-weight:500;cursor:pointer;white-space:nowrap")}><Icon name="plus" size={11}/>Add evidence</button>}</div>}
          </div>
          <span style={css("font-size:.61rem;font-weight:500;color:" + t.color + ";background:" + t.bg + ";border-radius:999px;padding:.16rem .42rem;white-space:nowrap;height:fit-content")}>{t.label}</span>
        </div>; })}</div>
      </section>;
    })}

    <Panel style="padding:.9rem 1.1rem"><div style={css("display:flex;align-items:center;justify-content:space-between;gap:.8rem;flex-wrap:wrap") }><div><h3 style={css("margin:0;font-size:.8rem;font-weight:500")}>{ready ? "Audit checks complete" : blockCount ? "Resolve the failed checks" : "Complete the evidence review"}</h3><p style={css("margin:.22rem 0 0;font-size:.62rem;color:var(--fg-faint)")}>{ready ? "The audit is ready for its report and planning stages." : blockCount ? `${blockCount} failed check${blockCount === 1 ? "" : "s"} must be addressed or accepted explicitly.` : `${needs.length} check${needs.length === 1 ? "" : "s"} still need review or connected data.`}</p></div><button type="button" disabled={!ready} onClick={onContinue} style={css("min-height:2.2rem;padding:0 .82rem;border:none;border-radius:999px;background:" + (ready ? "var(--accent)" : "var(--surface-alt)") + ";color:" + (ready ? "#fff" : "var(--fg-faint)") + ";font-size:.67rem;font-weight:500;cursor:" + (ready ? "pointer" : "not-allowed"))}>Continue to audit report →</button></div></Panel>
  </div>;
}
...
function SeoReadinessScoreCard({ category }: { category: SeoReadinessCategoryResult }) {
  ...
  return <section style={css("border:1px solid var(--border-soft);border-radius:16px;background:var(--surface);overflow:hidden") }>
    <div style={css("padding:1rem 1.1rem;border-bottom:1px solid var(--border-soft)") }>
      <div style={css("display:flex;align-items:center;gap:.75rem") }><span style={css("width:2.6rem;height:2.6rem;border-radius:.62rem;display:grid;place-items:center;font-size:.95rem;font-weight:500;flex-shrink:0;background:color-mix(in srgb," + color + " 13%,var(--surface) 87%);color:" + color)}>{category.score}</span><div style={css("min-width:0;flex:1") }><div style={css("font-size:1rem;font-weight:500")}>{category.title}</div><div style={css("font-size:.72rem;color:var(--fg-muted);margin-top:.2rem")}>{category.description}</div></div><div style={css("flex:1;min-width:4rem;max-width:11rem") }><div style={css("height:.4rem;border-radius:999px;background:color-mix(in srgb," + color + " 13%,var(--surface-alt) 87%)") }><div style={css("height:100%;border-radius:999px;width:" + Math.max(2, category.score) + "%;background:" + color)}/></div></div></div>
      <div style={css("display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.75rem") }><span style={css("font-size:.66rem;font-weight:500;color:var(--success);background:var(--success-soft);border-radius:999px;padding:.2rem .5rem")}>{category.passed} Passed</span><span style={css("font-size:.66rem;font-weight:500;color:var(--danger);background:var(--danger-soft);border-radius:999px;padding:.2rem .5rem")}>{category.failed} Failed</span><span style={css("font-size:.66rem;font-weight:500;color:var(--warn);background:var(--warn-soft);border-radius:999px;padding:.2rem .5rem")}>{category.unverified} Unverified</span></div>
    </div>
    <div style={css("display:flex;flex-direction:column") }>{category.checks.map(check => { const meta = statusMeta(check.status); return <div key={check.id} style={css("display:grid;grid-template-columns:1.55rem minmax(0,1fr) auto;gap:.6rem;align-items:start;padding:.7rem 1rem;border-bottom:1px solid var(--border-soft)") }><span style={css("width:1.45rem;height:1.45rem;border-radius:50%;display:grid;place-items:center;background:" + meta.bg + ";color:" + meta.color + ";font-size:.75rem;font-weight:500")}>{meta.symbol}</span><div style={css("min-width:0") }><div style={css("font-size:.78rem;font-weight:500;line-height:1.4")}>{check.label}</div><div style={css("font-size:.7rem;color:var(--fg-muted);line-height:1.45;margin-top:.22rem")}>{check.evidence}</div><div style={css("font-size:.64rem;color:var(--fg-faint);margin-top:.18rem")}>{check.source}</div></div><span style={css("font-size:.61rem;font-weight:500;color:" + meta.color + ";background:" + meta.bg + ";border-radius:999px;padding:.16rem .42rem;white-space:nowrap")}>{meta.label}</span></div>; })}</div>
  </section>;
}

function AuditReportView({ client, rows, stats, clientView, mobile, onContinue }: { client: StudioClient; rows: CrawlRow[]; stats: SeoStats; clientView: boolean; mobile: boolean; onContinue: () => void }) {
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.85rem") }>
    <section>
      <article style={css("border:1px solid var(--border-soft);border-radius:16px;background:var(--surface);padding:1.15rem 1.25rem") }>
        <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem;flex-wrap:wrap") }><div><div style={css("font-size:.68rem;text-transform:uppercase;letter-spacing:.04em;color:#6b5bd2")}>Crawl report</div><div style={css("font-size:1.05rem;font-weight:500;margin-top:.2rem")}>Technical evidence</div><div style={css("font-size:.78rem;color:var(--fg-muted);line-height:1.5;margin-top:.3rem")}>{headline}</div></div><button type="button" aria-expanded={pagesOpen} onClick={() => setPagesOpen(value => !value)} className="pt-softbtn" style={css("min-height:2rem;padding:0 .75rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);color:var(--fg-muted);font-size:.64rem;font-weight:500;cursor:pointer;white-space:nowrap")}>{pagesOpen ? "Hide pages audited" : "Show pages audited"}</button></div>
        <div style={css("display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.55rem;margin-top:.8rem") }>{[["Site health", `${stats.health}/100`, reportHealthColor], ["URLs reviewed", String(rows.length), "var(--fg)"], ["Indexable", String(stats.indexable.length), "var(--success)"], ["Need attention", String(changedPages.length), changedPages.length ? "var(--warn)" : "var(--success)"]].map(([label, value, color]) => <div key={label} style={css("border:1px solid var(--border-soft);border-radius:.8rem;background:var(--surface-alt);padding:.65rem .75rem") }><div style={css("font-size:.64rem;color:var(--fg-faint)")}>{label}</div><div style={css("font-size:1.25rem;font-weight:500;margin-top:.12rem;color:" + color)}>{value}</div></div>)}</div>
      </article>
    </section>

    <AiVisibilityPanel rows={rows} compact/>

    {pagesOpen && <section style={css("border:1px solid var(--border-soft);border-radius:16px;background:var(--surface);padding:1.05rem 1.15rem") }><div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:1rem") }><div><div style={css("font-size:1rem;font-weight:500")}>Pages audited</div><div style={css("font-size:.76rem;color:var(--fg-muted);margin-top:.2rem")}>Every page used as crawl evidence is listed in one vertical register.</div></div><span style={css("font-size:.72rem;color:var(--fg-muted);white-space:nowrap")}>{rows.length} pages</span></div><div style={css("display:flex;flex-direction:column;gap:.42rem;margin-top:.8rem") }>{rows.map((row, index) => { const decision = pageDecisionFor(row); return <div key={`${row.url}-${index}`} style={css("display:flex;align-items:center;gap:.6rem;padding:.55rem .65rem;border-radius:.7rem;background:var(--surface-alt)") }><span style={css("width:1.35rem;height:1.35rem;border-radius:50%;display:grid;place-items:center;background:var(--surface);border:1px solid var(--border-soft);font-size:.65rem;color:var(--fg-muted);flex-shrink:0")}>{index + 1}</span><span title={row.url} style={css("min-width:0;flex:1;font-size:.76rem;color:var(--fg-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{row.url}</span><Pill tone={decision.tone}>{decision.action === "No-index" ? "Keep private" : decision.action}</Pill></div>; })}</div></section>}

    <div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(15rem,1fr));gap:.75rem") }>
      <Panel style="padding:1rem"><SectionTitle title="What happens to each page" sub="Hover or focus a segment to see every page in that recommendation."/><div style={css("display:flex;height:.85rem;border-radius:999px;background:var(--surface-alt);margin-top:1rem") }>{decisionMix.filter(item => item.count).map((item, index, visible) => <DataVizDatum key={item.action} label={item.label} value={`${item.count} page${item.count === 1 ? "" : "s"}`} lines={item.pages} align={index > visible.length / 2 ? "right" : "left"} style={"display:block;height:100%;width:" + (item.count / Math.max(rows.length, 1) * 100) + "%;background:" + item.color}/>)}</div><div style={css("display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.48rem;margin-top:.85rem") }>{decisionMix.map(item => <div key={item.action} style={css("display:flex;align-items:center;justify-content:space-between;gap:.45rem;padding:.48rem .55rem;border:1px solid var(--border-soft);border-radius:.65rem") }><span style={css("display:flex;align-items:center;gap:.38rem;font-size:.61rem;color:var(--fg-muted)") }><i style={css("width:.48rem;height:.48rem;border-radius:50%;background:" + item.color + ";flex-shrink:0")}/>{item.label}</span><strong style={css("font-size:.72rem;font-weight:500")}>{item.count}</strong></div>)}</div></Panel>
      <Panel style="padding:1rem"><SectionTitle title="Where attention is needed" sub="Hover or focus a bar to see the affected pages."/><div style={css("display:flex;flex-direction:column;gap:.65rem;margin-top:.9rem") }>{attention.map(item => <div key={item.label}><div style={css("display:flex;align-items:center;justify-content:space-between;gap:.6rem;margin-bottom:.24rem;font-size:.62rem") }><span style={css("color:var(--fg-muted)")}>{item.label}</span><strong style={css("font-weight:500")}>{item.value}</strong></div><div style={css("height:.48rem;border-radius:999px;background:var(--surface-alt)") }>{item.value > 0 && <DataVizDatum label={item.label} value={`${item.value} signal${item.value === 1 ? "" : "s"}`} lines={item.pages} align="right" style={"display:block;height:100%;width:" + (item.value / attentionMax * 100) + "%;min-width:.48rem;border-radius:999px;background:" + item.color}/>}</div></div>)}</div></Panel>
    </div>


    <Panel style="padding:1rem"><SectionTitle title="What we recommend next" sub="Three clear workstreams turn the audit into an action plan."/><div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(11.5rem,1fr));gap:.6rem;margin-top:.8rem") }>{[
    ...
    ].map((item, index) => <div key={item.title} style={css("padding:.82rem;border:1px solid var(--border-soft);border-radius:.82rem;background:var(--surface-alt)") }><div style={css("display:flex;align-items:center;justify-content:space-between;gap:.5rem") }><span aria-hidden="true" style={css("width:2rem;height:2rem;border-radius:50%;border:1px solid var(--border-soft);background:var(--surface);display:grid;place-items:center;flex:0 0 2rem;color:var(--accent)") }><Icon name={item.icon} size={14}/></span><Pill tone={item.tone}>{index + 1} · {item.number} page{item.number === 1 ? "" : "s"}</Pill></div><h3 style={css("margin:.62rem 0 .28rem;font-size:.75rem;font-weight:500")}>{item.title}</h3><p style={css("margin:0;font-size:.62rem;line-height:1.48;color:var(--fg-muted)")}>{item.detail}</p></div>)}</div></Panel>

    <Panel style="padding:1rem"><div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem;flex-wrap:wrap") }><SectionTitle title="Pages that need a decision" sub="The important changes are shown first, in plain language."/><Pill tone={changedPages.length ? "warn" : "success"}>{changedPages.length} to review</Pill></div><div style={css("display:flex;flex-direction:column;gap:.5rem;margin-top:.8rem") }>{!mobile && <div aria-hidden="true" style={css("display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.45fr) 6.8rem;gap:.8rem;padding:0 .8rem .08rem;font-size:.54rem;line-height:1.2;letter-spacing:.035em;text-transform:uppercase;color:var(--fg-faint)") }><span>Page</span><span>Finding &amp; next step</span><span style={css("text-align:right")}>Action</span></div>}{changedPages.map(({ row, decision }) => <div key={row.url} style={css("display:grid;grid-template-columns:" + (mobile ? "minmax(0,1fr) auto" : "minmax(0,1fr) minmax(0,1.45fr) 6.8rem") + ";gap:.8rem;align-items:center;padding:.72rem .8rem;border:1px solid var(--border-soft);border-radius:.75rem;background:var(--surface-alt);min-width:0") }><div style={css("min-width:0") }><strong title={row.url} style={css("display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.7rem;font-weight:500")}>{row.url}</strong><span style={css("display:block;margin-top:.18rem;font-size:.58rem;color:var(--fg-faint)")}>{row.statusCode || "No response"} · {row.indexability}</span></div><div style={css("min-width:0" + (mobile ? ";grid-column:1/-1;grid-row:2" : "")) }><p style={css("margin:0;font-size:.62rem;line-height:1.45;color:var(--fg-muted)")}>{decision.reason}</p><div style={css("margin-top:.25rem;font-size:.59rem;line-height:1.4;color:var(--fg-faint)") }><strong style={css("font-weight:500;color:var(--fg-muted)")}>Next step:</strong> {decision.target}</div></div><span style={css("justify-self:end" + (mobile ? ";grid-column:2;grid-row:1" : ""))}><Pill tone={decision.tone}>{decision.action === "No-index" ? "Keep private" : decision.action}</Pill></span></div>)}</div>{!changedPages.length && <div style={css("padding:1.4rem;text-align:center;color:var(--fg-faint);font-size:.68rem")}>No page changes are currently recommended.</div>}</Panel>

    <details style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);overflow:hidden") }><summary style={css("list-style:none;cursor:pointer;padding:.9rem 1rem;display:flex;align-items:center;justify-content:space-between;gap:.8rem") }><div><h3 style={css("margin:0;font-size:.78rem;font-weight:500")}>See all page decisions</h3><p style={css("margin:.2rem 0 0;font-size:.61rem;color:var(--fg-faint)")}>Open the complete page-by-page register, including pages that can stay as they are.</p></div><Pill tone="muted">{decisions.length} pages</Pill></summary><div style={css("border-top:1px solid var(--border-soft)") }><DataTable headers={["Page", "HTTP", "Search visibility", "Recommended action", "Next step", "Why"]} rows={decisions.map(({ row, decision }) => [row.url, row.statusCode || "—", row.indexability, <Pill key="action" tone={decision.tone}>{decision.action === "No-index" ? "Keep private" : decision.action}</Pill>, decision.target, decision.reason])}/></div></details>
    <details style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);overflow:hidden") }><summary style={css("list-style:none;cursor:pointer;padding:.9rem 1rem;display:flex;align-items:center;justify-content:space-between;gap:.8rem") }><div><h3 style={css("margin:0;font-size:.78rem;font-weight:500")}>Technical redirect & consolidation plan</h3><p style={css("margin:.2rem 0 0;font-size:.61rem;color:var(--fg-faint)")}>Implementation instructions for moved, duplicate, removed, or private pages.</p></div><Pill tone={redirectPlan.length ? "warn" : "success"}>{redirectPlan.length} actions</Pill></summary><div style={css("border-top:1px solid var(--border-soft)") }><DataTable headers={["Current page", "Action", "Destination / instruction", "Evidence", "Plan status"]} rows={redirectPlan.map(({ row, decision }) => [row.url, <Pill key="action" tone={decision.tone}>{decision.action}</Pill>, decision.target, rawCell(row, ["Redirect Type", "Closest Similarity Match", "No. Near Duplicates", "Meta Robots 1", "Indexability Status"]) || `${row.statusCode} · ${row.indexability}`, decision.action === "Delete" ? "Remove references" : decision.action === "No-index" ? "Confirm exclusion" : "Implement 301"])} /></div></details>
    {clientView ? <Panel style="padding:1rem;background:linear-gradient(135deg,var(--surface),var(--accent-soft))"><div style={css("display:flex;align-items:center;justify-content:space-between;gap:.9rem;flex-wrap:wrap") }><div><span style={css("font-size:.58rem;text-transform:uppercase;letter-spacing:.04em;color:var(--accent)")}>Your SEO plan is ready</span><h3 style={css("margin:.35rem 0 0;font-size:.84rem;font-weight:500")}>Keyword mapping, metadata, and architecture are prepared.</h3><p style={css("margin:.25rem 0 0;font-size:.64rem;line-height:1.45;color:var(--fg-muted);max-width:34rem")}>These priorities and the full delivery roadmap are ready for the studio to action.</p></div><Pill tone="success">Ready to ship</Pill></div></Panel> : <div style={css("display:flex;justify-content:flex-end") }><button type="button" onClick={onContinue} style={css("display:inline-flex;align-items:center;gap:.4rem;min-height:2.35rem;padding:0 1.1rem;border:none;border-radius:999px;background:var(--accent);color:#fff;font-size:.76rem;font-weight:500;cursor:pointer")}><Icon name="send" size={14}/>Share with client</button></div>} 
  </div>;
}
...
function KeywordPagePlanView({ rows, mobile }: { rows: CrawlRow[]; mobile: boolean }) {
  ...
  if (!keywords.length && !pageMap.length) return <Panel style="padding:1.25rem"><SectionTitle title="Keyword & page plan" sub="Import keyword, volume, intent, and target-page columns in the crawl CSV to populate this plan."/><p style={css("margin:.8rem 0 0;font-size:.7rem;color:var(--fg-muted);line-height:1.5")}>No keyword-planning fields were found in the current source. The audit will not invent search demand or rankings.</p></Panel>;
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.85rem") }>
    <Panel style="padding:1rem;background:linear-gradient(135deg,var(--surface),color-mix(in srgb,var(--accent) 7%,var(--surface-alt) 93%))">
      ...
      ].map(item => <div key={item.label} style={css("padding:.7rem .75rem;border:1px solid var(--border-soft);border-radius:.75rem;background:color-mix(in srgb,var(--surface) 92%,transparent 8%)") }><strong style={css("display:block;font-size:1rem;font-weight:500")}>{item.value}</strong><span style={css("display:block;margin-top:.15rem;font-size:.58rem;color:var(--fg-faint)")}>{item.label}</span></div>)}</div>
    </Panel>

    <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "minmax(0,1.25fr) minmax(15rem,.75fr)") + ";gap:.85rem") }>
      <Panel style="overflow:hidden"><div style={css("padding:.9rem 1rem;border-bottom:1px solid var(--border-soft)") }><SectionTitle title="Keyword opportunities" sub="Prioritized using the imported demand, difficulty, intent, and target-page fields."/></div><div style={css("display:flex;flex-direction:column") }>{keywords.map((row,index) => <div key={`${row.keyword}-${row.page}`} style={css("display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:.65rem;align-items:center;padding:.72rem .8rem;border-bottom:1px solid var(--border-soft)") }><span style={css("width:1.55rem;height:1.55rem;border-radius:50%;background:var(--accent-soft);color:var(--accent);display:grid;place-items:center;font-size:.58rem;font-weight:500")}>{index + 1}</span><div style={css("min-width:0") }><div style={css("display:flex;align-items:center;gap:.38rem;flex-wrap:wrap") }><strong style={css("font-size:.7rem;font-weight:500")}>{row.keyword}</strong><Pill tone={row.intent.toLowerCase() === "commercial" ? "accent" : "muted"}>{row.intent}</Pill></div><div title={row.page} style={css("margin-top:.22rem;display:flex;align-items:center;gap:.28rem;font-size:.59rem;color:var(--fg-faint);min-width:0") }><Icon name="file" size={11}/><span style={css("overflow:hidden;text-overflow:ellipsis;white-space:nowrap")}>{row.page}</span></div></div><div style={css("display:grid;grid-template-columns:repeat(3,auto);gap:.58rem;text-align:right") }><div><strong style={css("display:block;font-size:.68rem;font-weight:500")}>{row.rank ? `#${row.rank}` : "—"}</strong><span style={css("font-size:.51rem;color:var(--fg-faint)")}>rank</span></div><div><strong style={css("display:block;font-size:.68rem;font-weight:500")}>{row.volume.toLocaleString()}</strong><span style={css("font-size:.51rem;color:var(--fg-faint)")}>volume</span></div><div><strong style={css("display:block;font-size:.68rem;font-weight:500;color:" + (row.difficulty > 50 ? "var(--warn)" : "var(--success)"))}>{row.difficulty || "—"}</strong><span style={css("font-size:.51rem;color:var(--fg-faint)")}>difficulty</span></div></div></div>)}</div></Panel>

      <Panel style="padding:1rem"><SectionTitle title="Opportunity matrix" sub="Hover or focus a point to see its imported keyword evidence."/><div style={css("height:15rem;position:relative;margin-top:.8rem;border:1px solid var(--border-soft);border-radius:.75rem;background:linear-gradient(90deg,transparent 49.8%,var(--border-soft) 50%,transparent 50.2%),linear-gradient(0deg,transparent 49.8%,var(--border-soft) 50%,transparent 50.2%)") }>{keywords.map((row,index) => <DataVizDatum key={`${row.keyword}-${row.page}`} label={row.keyword} value={`${row.volume.toLocaleString()} monthly searches`} lines={[`Landing page · ${row.page}`, `Current rank · ${row.rank ? `#${row.rank}` : "Not supplied"}`, `Difficulty · ${row.difficulty || "Not supplied"}`, `Intent · ${row.intent}`]} align={row.volume / maxVolume > .65 ? "right" : "left"} style={"position:absolute;left:" + Math.max(10, Math.min(90, row.volume / maxVolume * 82 + 7)) + "%;top:" + Math.max(10, Math.min(88, (row.difficulty || 50) * 1.22)) + "%;transform:translate(-50%,-50%);width:1.65rem;height:1.65rem;border-radius:50%;background:var(--accent);border:2px solid var(--surface);display:grid;place-items:center;color:#fff;font-size:.56rem;font-weight:500"}>{index + 1}</DataVizDatum>)}</div></Panel>
    </div>

    <Panel style="overflow:hidden"><div style={css("padding:.9rem 1rem;border-bottom:1px solid var(--border-soft);display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem;flex-wrap:wrap") }><SectionTitle title="Keyword-to-page map" sub="The imported current page, focused keyword, and final destination in one register."/><Pill tone="accent">{pageMap.length} page decisions</Pill></div><div style={css("display:flex;flex-direction:column;gap:.5rem;padding:.65rem") }>{pageMap.map((row,index) => <div key={`${row.current}-${row.keyword}`} style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "auto minmax(0,.8fr) auto minmax(0,1fr) minmax(9rem,.85fr) auto") + ";gap:.65rem;align-items:center;padding:.72rem .75rem;border:1px solid var(--border-soft);border-radius:.75rem;background:var(--surface-alt)") }><span style={css("width:1.45rem;height:1.45rem;border-radius:50%;background:var(--surface);display:grid;place-items:center;font-size:.55rem;color:var(--fg-faint)")}>{index + 1}</span><div style={css("min-width:0") }><span style={css("display:block;font-size:.52rem;text-transform:uppercase;color:var(--fg-faint)")}>Current page</span><strong style={css("display:block;margin-top:.16rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.66rem;font-weight:500")}>{row.current}</strong></div>{!mobile && <span aria-hidden="true">→</span>}<div style={css("min-width:0") }><span style={css("display:block;font-size:.52rem;text-transform:uppercase;color:var(--fg-faint)")}>Final destination</span><strong style={css("display:block;margin-top:.16rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.66rem;font-weight:500;color:var(--accent)")}>{row.proposed}</strong></div><div style={css("min-width:0") }><span style={css("display:block;font-size:.52rem;text-transform:uppercase;color:var(--fg-faint)")}>Focused keyword</span><span style={css("display:block;margin-top:.16rem;font-size:.62rem;color:var(--fg-muted)")}>{row.keyword || "Not supplied"}</span></div><Pill tone={row.status === "Keep" ? "success" : row.status === "New" ? "accent" : "warn"}>{row.status}</Pill></div>)}</div></Panel>
  </div>;
}
...
function RoadmapSummaryView({ mobile = false }: { mobile?: boolean }) {
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.7rem") }>{phases.map((phase, pi) => <Panel key={phase.name} style="padding:1.1rem 1.2rem">
    <div style={css("display:flex;align-items:center;gap:.55rem;flex-wrap:wrap") }><span style={css("width:1.7rem;height:1.7rem;border-radius:50%;flex-shrink:0;display:grid;place-items:center;font-size:.74rem;font-weight:500;background:color-mix(in srgb," + phase.color + " 14%,var(--surface) 86%);color:" + phase.color)}>{pi + 1}</span><h3 style={css("margin:0;font-size:.95rem;font-weight:500;line-height:1.2")}>{phase.name}</h3><Pill tone={phase.tone}>{phase.time}</Pill><span style={css("font-size:.56rem;text-transform:uppercase;letter-spacing:.04em;color:var(--fg-faint);white-space:nowrap")}>{phase.focus}</span></div>
    <p style={css("margin:.5rem 0 0;font-size:.7rem;line-height:1.55;color:var(--fg-muted);max-width:44rem")}>{phase.objective}</p>
    <div style={css("display:flex;flex-direction:column;gap:.5rem;margin-top:.9rem") }>{phase.actions.map((action, index) => <div key={action.title} style={css("display:grid;grid-template-columns:1.35rem minmax(0,1fr);gap:.6rem;align-items:start;padding:.68rem .78rem;border:1px solid var(--border-soft);border-radius:.72rem;background:var(--surface-alt)") }><span style={css("width:1.35rem;height:1.35rem;border-radius:50%;background:var(--surface);border:1px solid color-mix(in srgb," + phase.color + " 30%,var(--border-soft) 70%);display:grid;place-items:center;font-size:.6rem;font-weight:500;color:" + phase.color + ";flex-shrink:0")}>{index + 1}</span><div style={css("min-width:0") }><div style={css("font-size:.74rem;font-weight:500;line-height:1.3")}>{action.title}</div><p style={css("margin:.2rem 0 0;font-size:.65rem;line-height:1.5;color:var(--fg-muted)")}>{action.detail}</p></div></div>)}</div>
    <div style={css("display:flex;align-items:flex-start;gap:.5rem;margin-top:.75rem;padding:.62rem .78rem;border-radius:.72rem;background:color-mix(in srgb," + phase.color + " 8%,var(--surface) 92%)") }><span style={css("color:" + phase.color + ";flex-shrink:0;margin-top:.02rem")}><Icon name="checkmark" size={13}/></span><p style={css("margin:0;font-size:.66rem;line-height:1.5;color:var(--fg)") }><strong style={css("font-weight:500")}>Outcome</strong> — {phase.outcome}</p></div>
  </Panel>)}</div>;
}
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.8rem") }>
    <Panel style="padding:1rem 1.1rem">
      ...
      <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "minmax(0,.85fr) minmax(0,1.15fr)") + ";gap:1.1rem;margin-top:.85rem;align-items:start") }>
        <div>
          <div style={css("font-size:.58rem;font-weight:500;text-transform:uppercase;letter-spacing:.04em;color:var(--fg-faint);margin-bottom:.55rem")}>By phase</div>
          ...
          <div style={css("display:flex;flex-direction:column;gap:.42rem;margin-top:.65rem") }>{phaseMeta.map(p => <div key={p.phase} style={css("display:flex;align-items:center;justify-content:space-between;gap:.6rem") }><span style={css("display:flex;align-items:center;gap:.42rem;font-size:.68rem;color:var(--fg-muted)") }><span style={css("width:.5rem;height:.5rem;border-radius:50%;background:" + p.color)}/>{p.phase}</span><strong style={css("font-size:.74rem;font-weight:500;font-variant-numeric:tabular-nums")}>{p.count}</strong></div>)}</div>
        </div>
        <div>
          <div style={css("font-size:.58rem;font-weight:500;text-transform:uppercase;letter-spacing:.04em;color:var(--fg-faint);margin-bottom:.55rem")}>By workstream</div>
          <div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,7.5rem),1fr));gap:.45rem") }>{workstreams.map(item => <div key={item.workstream} style={css("padding:.55rem .62rem;border:1px solid var(--border-soft);border-radius:.68rem;background:var(--surface-alt)") }><div style={css("font-size:.95rem;font-weight:500;font-variant-numeric:tabular-nums")}>{item.count}</div><div style={css("margin-top:.1rem;font-size:.6rem;color:var(--fg-faint)")}>{item.workstream}</div></div>)}</div>
        </div>
      </div>
    </Panel>
    ...
    {showToast && <Panel style="padding:.9rem"><div style={css("display:flex;align-items:center;justify-content:space-between;gap:.8rem;flex-wrap:wrap") }><div><h3 style={css("margin:0;font-size:.82rem;font-weight:500")}>Ready to turn the complete plan into delivery?</h3><p style={css("margin:.25rem 0 0;font-size:.65rem;color:var(--fg-faint)")}>Create all {planItems.length} listed actions in the studio to-do board.</p></div><button type="button" onClick={() => showToast(`${planItems.length} SEO plan tasks prepared for import`)} style={css("height:2.2rem;padding:0 .85rem;border:none;border-radius:999px;background:var(--accent);color:#fff;font-size:.68rem;font-weight:500;cursor:pointer")}>Create all delivery tasks</button></div></Panel>}
  </div>;
}

function DataTable({ headers, rows }: { headers: string[]; rows: ReactNode[][] }) { return <div style={css("overflow:auto") }><table style={css("width:100%;border-collapse:collapse;min-width:42rem") }><thead><tr>{headers.map(header => <th key={header} style={css("padding:.65rem .75rem;text-align:left;font-size:.58rem;font-weight:500;color:var(--fg-faint);background:var(--surface-alt);border-bottom:1px solid var(--border-soft);white-space:nowrap")}>{header}</th>)}</tr></thead><tbody>{rows.map((row,rowIndex) => <tr key={rowIndex}>{row.map((value,index) => <td key={index} style={css("padding:.68rem .75rem;border-bottom:1px solid var(--border-soft);font-size:.65rem;color:" + (index === 0 ? "var(--fg)" : "var(--fg-muted)") + ";max-width:17rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{value}</td>)}</tr>)}</tbody></table>{!rows.length && <div style={css("padding:2.5rem;text-align:center;color:var(--fg-faint);font-size:.72rem")}>No matching rows.</div>}</div>; }
