  ...
  return <div style={css("width:100%;padding:" + (state.isMobile ? "1rem .9rem 1.5rem" : "1.4rem 1.5rem"))}>
    <div style={css("width:100%;max-width:60rem;margin:0 auto;display:flex;flex-direction:column;gap:.85rem;box-sizing:border-box") }>
      <div style={css("display:flex;align-items:center;gap:var(--space-3);flex-wrap:wrap") }>
        {state.role !== "client" && <><button type="button" onClick={() => setSelectedClient(null)} className="pt-softbtn" style={css("border:1px solid var(--border);border-radius:var(--radius-pill);background:var(--surface);color:var(--fg-muted);padding:.4rem .8rem;font-size:var(--text-xs);cursor:pointer")}>← All checkups</button><div style={{ minWidth: 0 }}><span style={css("font-size:var(--text-lg);font-weight:500")}>SEO Audit</span><span style={css("font-size:var(--text-base);color:var(--fg-muted)")}> · {selectedClient.name}</span></div></>}
        {state.role !== "client" && <button type="button" onClick={() => setResetClient(selectedClient)} className="pt-softbtn" style={css("margin-left:auto;display:inline-flex;align-items:center;gap:.35rem;border:1px solid var(--border);border-radius:var(--radius-pill);background:var(--surface);color:var(--fg-muted);padding:.4rem .8rem;font-size:var(--text-2xs);font-weight:500;cursor:pointer")}><Icon name="replay" size={13}/>Start over</button>}
      </div>
