    ...
    <section data-report-task-plan aria-labelledby={headingId} style={css(embedded
      ...
      <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem;flex-wrap:wrap;padding:" + (embedded ? "1.2rem 1.4rem" : "1rem 1.1rem") + ";border-bottom:1px solid var(--border-soft);background:" + (embedded ? "var(--surface)" : "color-mix(in srgb,var(--accent-soft) 38%,var(--surface) 62%)")) }>
        <div style={{ minWidth: 0 }}>
          ...
          <p data-report-exclude style={css("margin:.3rem 0 0;font-size:.73rem;line-height:1.45;color:var(--fg-muted)")}>Edit the names, choose what to include, then import directly or download the same rows as CSV.</p>
          <p data-report-print-only style={css("display:none;margin:.3rem 0 0;font-size:.73rem;line-height:1.45;color:var(--fg-muted)")}>Build-ready checklist derived from the approved funnel plan.</p>
        </div>
        ...
        {rows.map((row, index) => {
          ...
          return (
            <label data-report-task-row key={key} style={css("display:grid;grid-template-columns:" + (mobile ? "1.2rem minmax(0,1fr)" : "1.2rem minmax(0,1fr) 9.5rem") + ";gap:.55rem;align-items:center;min-height:2.55rem;padding:.38rem .68rem;border:1px solid " + (checked ? "color-mix(in srgb,var(--accent) 35%,var(--border-soft) 65%)" : "var(--border-soft)") + ";border-radius:999px;background:" + (checked ? "color-mix(in srgb,var(--accent-soft) 30%,var(--surface) 70%)" : "var(--surface)") + ";cursor:pointer") }>
              ...
              <span style={css("display:flex;flex-direction:column;justify-content:center;gap:.25rem;min-width:0;align-self:stretch") }>
                <input value={row.title} onClick={event => event.stopPropagation()} onChange={event => updateRow(index, { title: event.currentTarget.value })} aria-label={`Task ${index + 1} name`} style={css("width:100%;min-width:0;border:0;border-bottom:1px solid transparent;background:transparent;color:var(--fg);font:inherit;font-size:.76rem;font-weight:500;line-height:1.1;outline:none;padding:.08rem 0")}/>
                {mobile && <span style={css("font-size:.64rem;color:var(--fg-faint);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")}>{row.milestone || "Implementation"}</span>}
              </span>
              {!mobile && <span style={css("display:inline-flex;align-items:center;gap:.35rem;font-size:.66rem;color:var(--fg-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis") }><span style={css("width:.45rem;height:.45rem;border-radius:50%;background:var(--accent);flex-shrink:0")}/>{row.milestone || "Implementation"}</span>}
            </label>
          );
        })}
      </div>
