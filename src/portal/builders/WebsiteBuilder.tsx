...
const WIZARD: DiscoveryTopic[] = [
  { id: "context", num: "01", title: "Business & redesign context", icon: "grid", qs: [
    ...
    { key: "url", label: "What is the current website URL?", kind: "text", ph: "https://brand.com" },
    { key: "redesignReason", label: "Why is the website being redesigned now?", kind: "textarea", ph: "What has changed, what is not working, and why now?" },
  ] },
  ...
  { id: "scope", num: "03", title: "Pages & scope", icon: "file", qs: [
    { key: "mustKeep", label: "Which existing pages or content must be preserved?", hint: "List each page, URL, resource, or content group separately.", kind: "textarea", list: true, ph: "• Homepage\n• Top-performing service pages\n• Legal and policy pages\n• High-ranking articles" },
    { key: "newPages", label: "Which new pages or sections are already expected?", kind: "textarea", ph: "Proposed additions beyond the current sitemap…" },
  ...
  { id: "content", num: "05", title: "Content, copy & delivery", icon: "msg", qs: [
    { key: "contentOwner", label: "Who will provide existing content and assets?", kind: "single", opts: ["Client", "Studio", "Shared responsibility", "Not decided"] },
    { key: "copyScope", label: "Should copy be included in this build?", kind: "single", opts: ["No — structure only", "Optional add-on", "Light editing only", "Full website copy"] },
...
const STAGES: DiscoveryStage[] = [
  ...
  { key: "direction", label: "Build direction", icon: "chart" },
...
const INTRO: DiscoveryIntroStep[] = [
  ...
  { title: "Sitemap inventory", tag: "Every current page", icon: "file" },
  { title: "Functional scope", tag: "Features & systems", icon: "activity" },
  { title: "Content responsibility", tag: "Copy is optional", icon: "msg" },
...
function HomepageDirection({ result, mobile }: { result: NonNullable<ReturnType<typeof getAiResult>>; mobile: boolean }) {
  ...
  return <article style={css("max-width:48rem;margin:0 auto;border:1px solid var(--border-soft);border-radius:1rem;background:var(--surface);padding:" + (mobile ? "1.15rem" : "1.65rem 1.8rem") + ";display:flex;flex-direction:column;gap:1.25rem") }>
    <header>
      <div style={css("display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap") }><span style={css("font-size:.68rem;text-transform:uppercase;letter-spacing:.04em;color:var(--accent)")}>Website development plan</span><span style={css("font-size:.68rem;color:var(--accent);background:var(--accent-soft);border-radius:999px;padding:.25rem .65rem")}>For owner approval</span></div>
...
const PIPELINE: Pipeline = {
  ...
  genPrompt: key => key === "direction" ? "Create the complete website design and development plan: objective, tone, rebuild changes, every page in scope, the full-site design system, homepage approval reference, and prioritized milestones." : "Turn the approved full-site direction, homepage reference, and complete page scope into an implementation task list.",
  genCta: key => key === "direction" ? "Create build direction" : "Create task plan",
  approveLabel: (_key, last) => last ? "Approve & create tasks →" : "Approve & continue →",
  beginLabel: "Create build direction →",
  beginMsg: () => "I have everything I need to inventory the current sitemap and prepare the redesign direction.",
  ...
  renderProposal: ctx => {
    ...
    return <section style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:var(--surface);overflow:hidden") }>
      <div style={css("padding:1.4rem 1.5rem") }><div style={css("font-size:.68rem;text-transform:uppercase;letter-spacing:.04em;color:var(--accent)")}>Builder complete</div><h2 style={css("margin:.3rem 0 0;font-size:1.3rem;font-weight:500")}>Build direction and tasks are ready</h2><p style={css("margin:.45rem 0 0;font-size:.8rem;color:var(--fg-muted)")}>The sitemap inventory, redesign direction, optional copy scope, and implementation tasks are ready to use.</p><button type="button" onClick={ctx.onExit} className="pt-softbtn" style={css("margin-top:1rem;height:2.4rem;padding:0 1.1rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);color:var(--fg-muted);font-size:.78rem;font-weight:500;cursor:pointer")}>Back to builders</button></div>
...
export function WebsiteBuilder({ state, actions }: { state: PortalState; actions: PortalActions }) {
  ...
  const openClient = (item: StudioClient) => { setBuilderKnow({ data: {}, sources: {} }); setClient(item); };
  const cards = useMemo(() => availableClients.map(item => ({ id: item.id, name: item.name, subtitle: "Website build direction", statusLabel: "Not started", statusTone: "muted" as const, stage: "Website builder", progress: 0, owner: item.owner, due: "—", headerAction: { label: `Create website build for ${item.name}`, icon: "plus", onClick: () => openClient(item) }, showStatus: false, showProgress: false, showStage: false, showMeta: false, showFooter: false, hero: <FunnelFlowHero direction="Website redesign" goal="Complete rebuild direction" build="Not started" readyCount={0} />, primaryLabel: "Open builder", onPrimary: () => openClient(item), secondaryLabel: "New build", secondaryIcon: "plus", onSecondary: () => openClient(item) })), [availableClients]);
