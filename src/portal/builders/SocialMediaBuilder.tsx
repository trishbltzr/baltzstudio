...
const STAGES: Array<{ id: Stage; label: string; note: string; icon: string }> = [
  { id: "brief", label: "Brief", note: "Jump-start from your current posts", icon: "inbox" },
  { id: "plan", label: "Content plan", note: "Approve the month at a glance", icon: "layers" },
  { id: "calendar", label: "Posts & calendar", note: "Review and approve each post on the month", icon: "cal" },
  { id: "schedule", label: "Schedule", note: "Ship it or share it", icon: "send" },
];
...
const inputStyle = "width:100%;box-sizing:border-box;border:1px solid var(--border);border-radius:.8rem;background:var(--surface);color:var(--fg);padding:.7rem .78rem;font:inherit;font-size:.78rem;outline:none";
const buttonPrimary = "display:inline-flex;align-items:center;justify-content:center;gap:.4rem;min-height:2.35rem;padding:0 .95rem;border:none;border-radius:999px;background:var(--accent);color:#fff;font-size:.76rem;font-weight:500;cursor:pointer";
const buttonSoft = "display:inline-flex;align-items:center;justify-content:center;gap:.38rem;min-height:2.2rem;padding:0 .82rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);color:var(--fg-muted);font-size:.73rem;font-weight:500;cursor:pointer";
...
function socialMonthStatus(project: SocialProject) {
  ...
  if (project.sent) return { label: "Scheduled", tone: "success" as const, stage: `${project.posts.length} posts scheduled` };
...
function ChannelMarks({ ids, size = 1.2, logoSize = 10, max = 3 }: { ids: ChannelId[]; size?: number; logoSize?: number; max?: number }) {
  ...
  return <span aria-label={ids.map(id => channel(id).label).join(", ")} title={ids.map(id => channel(id).label).join(", ")} style={css("display:inline-flex;align-items:center;padding-left:" + (visible.length > 1 ? ".22rem" : "0"))}>
    ...
    {ids.length > max && <span style={css(`width:${size}rem;height:${size}rem;margin-left:-.22rem;border-radius:50%;display:grid;place-items:center;background:var(--surface);border:1px solid var(--border);font-size:.48rem;color:var(--fg-muted)`)}>+{ids.length - max}</span>}
  </span>;
}
...
function SocialCalendarHero() {
  return <div style={css("border:1px solid var(--border-soft);border-radius:.82rem;background:var(--surface-alt);padding:.62rem") }>
    ...
    <div style={css("display:flex;align-items:center;justify-content:space-between;margin-top:.5rem;font-size:.62rem;color:var(--fg-faint)")}><span>Voice → plan → calendar</span><span style={css("color:var(--accent)")}>Ready to build</span></div>
  </div>;
}
  ...
  return <Panel style="overflow:hidden">
    <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "minmax(0,1fr) minmax(0,.86fr)") + ";align-items:stretch") }>
      <div style={css("padding:" + (mobile ? "1.35rem 1.2rem 1.45rem" : "1.9rem 1.8rem 2rem") + ";display:flex;flex-direction:column") }>
        <div style={css("display:flex;align-items:center;gap:.5rem;font-size:.62rem;font-weight:500;letter-spacing:.08em;text-transform:uppercase;color:var(--accent)")}><span style={css("width:.38rem;height:.38rem;border-radius:50%;background:var(--accent)")}/>Baltz Studio · Content engine</div>
        <h2 style={css("margin:.75rem 0 0;font-size:1.55rem;font-weight:500;line-height:1.12;letter-spacing:-.02em")}>A month of content,<br/>mapped and ready.</h2>
        <p style={css("margin:.6rem 0 0;font-size:.86rem;color:var(--fg-muted);line-height:1.5;max-width:24rem")}>Paste what you already post. We draft the month, you approve it layer by layer, and it lands on a calendar ready to schedule.</p>
        <div style={css("margin-top:1.2rem;border:1px solid var(--border-soft);border-radius:var(--radius);overflow:hidden")}>{INTRO_STEPS.map((step, index) => <div key={step.title} style={css("display:flex;align-items:center;gap:.7rem;padding:.62rem .85rem;background:var(--surface)" + (index < INTRO_STEPS.length - 1 ? ";border-bottom:1px solid var(--border-soft)" : ""))}><span style={css("width:1.85rem;height:1.85rem;border-radius:8px;background:var(--accent-soft);color:var(--accent);display:grid;place-items:center;flex-shrink:0")}><Icon name={step.icon} size={15}/></span><span style={css("flex:1;min-width:0;font-size:.83rem;font-weight:500")}>{step.title}</span><span style={css("font-size:.68rem;color:var(--fg-faint);white-space:nowrap")}>{step.tag}</span><span style={css("font-size:.6rem;color:var(--fg-faint);width:.9rem;text-align:right")}>{step.num}</span></div>)}</div>
      ...
      <div style={css("background:var(--surface-alt);border-left:" + (mobile ? "none" : "1px solid var(--border-soft)") + ";border-top:" + (mobile ? "1px solid var(--border-soft)" : "none") + ";min-height:" + (mobile ? "22rem" : "30rem") + ";padding:1.6rem 1.5rem") }>
        <div style={css("background:#fff;border:1px solid var(--border-soft);border-radius:12px;box-shadow:0 20px 50px -30px rgba(60,40,30,.5);overflow:hidden") }><div style={css("padding:.75rem .9rem;border-bottom:1px solid var(--border-soft);display:flex;align-items:center;justify-content:space-between") }><span style={css("font-size:.82rem;font-weight:500")}>May · 16 posts</span><span style={css("font-size:.62rem;color:var(--fg-faint)")}>3 channels</span></div><div style={css("display:grid;grid-template-columns:repeat(7,1fr);padding:.5rem .6rem .7rem;gap:.28rem")}>{Array.from({ length: 21 }, (_, index) => <div key={index} style={css("aspect-ratio:1;border-radius:5px;background:" + (index % 5 === 0 ? "var(--accent-soft)" : "var(--surface-alt)") + ";padding:.2rem;position:relative") }><span style={css("font-size:.5rem;color:var(--fg-faint)")}>{index + 1}</span>{[1, 4, 7, 10, 13, 16, 19].includes(index) && <span style={css("position:absolute;bottom:.2rem;left:.2rem;right:.2rem;height:.32rem;border-radius:2px;background:" + CHANNELS[index % 3].color)}/>}</div>)}</div></div>
        <div style={css("margin-top:.9rem;background:#fff;border:1px solid var(--border-soft);border-radius:12px;box-shadow:0 16px 40px -28px rgba(60,40,30,.5);padding:.85rem .95rem") }><div style={css("display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem") }><span style={css("width:1.4rem;height:1.4rem;border-radius:6px;background:#d86478;color:#fff;display:grid;place-items:center;font-size:.56rem;font-weight:500")}>Ig</span><span style={css("font-size:.6rem;font-weight:500;padding:.1rem .45rem;border-radius:999px;background:var(--accent-soft);color:var(--accent)")}>Reel · 9:16</span><span style={css("font-size:.6rem;color:var(--fg-faint);margin-left:auto")}>Wed 12:00pm</span></div><div style={css("font-size:.78rem;font-weight:500")}>A day behind the brand</div><p style={css("margin:.25rem 0 0;font-size:.68rem;color:var(--fg-muted);line-height:1.5")}>Come behind the scenes for a look at the people, process, and thinking behind the finished work.</p><div style={css("font-size:.62rem;color:var(--accent);margin-top:.4rem")}>#behindthescenes #brandstory #founder</div></div>
      </div>
    </div>
  </Panel>;
}
...
export function SocialMediaBuilder({ state, actions }: { state: PortalState; actions: PortalActions }) {
  ...
  const cards = useMemo(() => availableClients.filter(item => (savedMonths[item.id] || []).length > 0).map(item => {
    ...
    return {
      ...
      stats: [{ label: "Months", value: String(months.length), tone: months.length ? "accent" as const : "default" as const }, { label: "Planned posts", value: String(totalPosts) }, { label: "Latest", value: latest ? formatMonthKey(latest.monthKey).replace(" 20", " ’") : "—" }],
      compactDetails: true,
...
function SocialWorkspace({ client, monthId, months, mobile, hideIdentity, actions, onSelectMonth, onCreateMonth, onExit }: { client: StudioClient; monthId: string; months: SocialMonthRecord[]; mobile: boolean; hideIdentity: boolean; actions: PortalActions; onSelectMonth: (monthId: string) => void; onCreateMonth: () => void; onExit: () => void }) {
  ...
  const stageStatus = project.stage === "calendar" && approved === project.posts.length ? "Approved" : project.stage === "schedule" ? project.sent ? "Scheduled" : "Ready to ship" : "Ready for review";
      ...
      <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "14rem minmax(0,1fr)") + ";gap:.85rem;align-items:start") }>
        <aside style={css("border:1px solid var(--border-soft);border-radius:var(--radius-panel);background:linear-gradient(180deg,var(--surface),color-mix(in srgb,var(--surface-alt) 78%,white 22%));padding:.9rem .7rem .7rem;" + (mobile ? "overflow-x:auto" : "position:sticky;top:.5rem"))}>
          <div style={css("padding:0 .4rem .8rem;min-width:" + (mobile ? "38rem" : "0")) }>
            <div style={css("display:flex;align-items:center;gap:.55rem") }><span style={css("width:1.7rem;height:1.7rem;border-radius:8px;background:var(--accent);color:#fff;display:grid;place-items:center;flex-shrink:0") }><Icon name="layers" size={14}/></span><div><div style={css("font-size:.92rem;font-weight:500;line-height:1.1")}>Content pipeline</div><div style={css("font-size:.68rem;color:var(--fg-faint);margin-top:.1rem")}>{STAGES.length} stages to schedule</div></div></div>
            <div style={css("display:flex;align-items:center;gap:.55rem;margin-top:.75rem") }><div style={css("flex:1;height:.35rem;border-radius:999px;background:var(--border-soft);overflow:hidden") }><div style={css("height:100%;border-radius:999px;background:var(--success);width:" + Math.max(2, doneCount / STAGES.length * 100) + "%;transition:width .35s ease")}/></div><span style={css("font-size:.68rem;font-weight:500;color:var(--fg-muted)")}>{doneCount}/{STAGES.length}</span></div>
          </div>
            ...
            {STAGES.map((item, index) => {
              ...
              return <button key={item.id} type="button" aria-current={active ? "step" : undefined} disabled={!enabled} onClick={() => enabled && patch({ stage: item.id })} style={css("display:flex;align-items:center;gap:.6rem;" + (mobile ? "flex-direction:column;min-width:9.2rem;text-align:center;padding:.35rem .3rem;" : "padding:.15rem .4rem;") + "border:none;border-radius:8px;background:" + (active ? "var(--accent-soft)" : "transparent") + ";text-align:left;cursor:" + (enabled ? "pointer" : "default") + ";opacity:" + (enabled ? "1" : ".55"))}>
                <span style={css("position:relative;display:flex;flex-direction:column;align-items:center;flex-shrink:0")}>
                  ...
                  <span style={css("width:1.15rem;height:1.15rem;border-radius:50%;display:grid;place-items:center;flex-shrink:0;font-size:.62rem;font-weight:500;background:" + dotBg + ";border:1px solid " + dotBorder + ";color:" + dotColor)}>{done ? <Icon name="checkmark" size={9}/> : index + 1}</span>
                  {!mobile && <span aria-hidden="true" style={css("width:2px;height:.5rem;background:" + (index === STAGES.length - 1 ? "transparent" : "var(--border)"))}/>}
                </span>
                <span style={css("font-size:.82rem;font-weight:" + (active ? "500" : "400") + ";color:" + (active ? "var(--fg)" : enabled ? "var(--fg-muted)" : "var(--fg-faint)"))}>{item.label}</span>
              </button>;
            })}
          </div>
        </aside>
        ...
        <main style={{ minWidth: 0 }}>
          ...
          {project.stage === "brief" && project.entered && <BriefStage clientName={client.name} project={project} patch={patch} analyze={analyze} buildPlan={buildPlan} mobile={mobile} aiBusy={aiBusy} aiError={aiError}/>} 
          {project.stage === "plan" && <StageShell stage="plan" clientName={client.name} status={stageStatus}><PlanStage project={project} patch={patch} mobile={mobile} rebuild={buildPlan} aiBusy={aiBusy}/></StageShell>} 
...
function SourceAnalyzer({ clientName, project, patch, analyze, mobile, busy, error }: { clientName: string; project: SocialProject; patch: (next: Partial<SocialProject>) => void; analyze: () => void; mobile: boolean; busy: boolean; error: string }) {
  ...
  return <Panel style="padding:1rem 1.05rem">
    <div style={css("display:flex;align-items:center;gap:.6rem") }>
      ...
      <div><h2 style={css("margin:0;font-size:.95rem;font-weight:500")}>Jump-start from what you already post</h2><p style={css("margin:.15rem 0 0;font-size:.74rem;color:var(--fg-muted)")}>We read your voice and pillars so you skip the questionnaire.</p></div>
    </div>
    ...
    <div style={css("margin-top:.85rem;border:1px solid var(--border);border-radius:.95rem;background:var(--surface);overflow:hidden") }>
      ...
      <div style={css("padding:.7rem .72rem .62rem") }>
        {project.source === "website" && <div style={css("display:flex;align-items:center;gap:.42rem;flex-wrap:wrap;margin-bottom:.55rem") }>
          <span style={css("font-size:.62rem;color:var(--fg-faint)")}>Choose a site:</span>
          <button type="button" onClick={() => patch({ sourceText: suggestedSite, analyzed: false })} aria-pressed={project.sourceText === suggestedSite} style={css("min-height:1.85rem;padding:0 .62rem;border:1px solid " + (project.sourceText === suggestedSite ? "var(--accent)" : "var(--border)") + ";border-radius:999px;background:" + (project.sourceText === suggestedSite ? "var(--accent-soft)" : "var(--surface)") + ";color:" + (project.sourceText === suggestedSite ? "var(--accent)" : "var(--fg-muted)") + ";font-size:.64rem;cursor:pointer")}>{clientName} website</button>
          <button type="button" onClick={() => patch({ sourceText: "", analyzed: false })} aria-pressed={project.sourceText !== suggestedSite} style={css("min-height:1.85rem;padding:0 .62rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);color:var(--fg-muted);font-size:.64rem;cursor:pointer")}>Enter another site</button>
        </div>}
        ...
        {error && <p role="alert" style={css("margin:.55rem .22rem 0;font-size:.68rem;color:var(--danger)")}>{error}</p>}
      </div>
    </div>

    {project.analyzed && <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "1fr 1.25fr") + ";gap:.65rem;margin-top:.75rem") }><div style={css("padding:.75rem;border:1px solid var(--border-soft);border-radius:.8rem;background:var(--surface-alt)")}><span style={css("font-size:.62rem;text-transform:uppercase;color:var(--fg-faint)")}>Detected voice</span><div style={css("display:flex;gap:.35rem;flex-wrap:wrap;margin-top:.5rem")}>{project.voice.map(item => <span key={item} style={css("padding:.24rem .52rem;border-radius:999px;background:var(--accent-soft);color:var(--accent);font-size:.65rem")}>{item}</span>)}</div></div><div style={css("padding:.75rem;border:1px solid var(--border-soft);border-radius:.8rem;background:var(--surface-alt)")}><span style={css("font-size:.62rem;text-transform:uppercase;color:var(--fg-faint)")}>Content pillars</span><div style={css("display:flex;gap:.35rem;flex-wrap:wrap;margin-top:.5rem")}>{project.pillars.map(item => <span key={item} style={css("padding:.24rem .52rem;border-radius:999px;background:var(--surface);border:1px solid var(--border-soft);font-size:.65rem")}>{item}</span>)}</div></div></div>}
  </Panel>;
}
...
function ChannelCadence({ project, patch, buildPlan, mobile, busy }: { project: SocialProject; patch: (next: Partial<SocialProject>) => void; buildPlan: () => void; mobile: boolean; busy: boolean }) {
  ...
  return <Panel style="overflow:hidden">
    <div style={css("padding:1rem 1.1rem;border-bottom:1px solid var(--border-soft)") }>
      <h2 style={css("margin:0;font-size:1.05rem;font-weight:500")}>Channels & cadence</h2>
      <p style={css("margin:.2rem 0 0;font-size:.76rem;color:var(--fg-muted)")}>Where we publish, how often, and for how long.</p>
    </div>
    <div style={css("padding:1.05rem 1.1rem 1.1rem") }>
      <span style={css("display:block;font-size:.64rem;font-weight:500;letter-spacing:.05em;text-transform:uppercase;color:var(--fg-faint)")}>Channels</span>
      <div style={css("display:flex;align-items:center;gap:.48rem;flex-wrap:wrap;margin-top:.62rem") }>
        {CHANNELS.map(item => {
          ...
          return <button key={item.id} type="button" aria-pressed={selected} onClick={() => patch({ channels: selected ? project.channels.filter(id => id !== item.id) : [...project.channels, item.id] })} style={css("display:inline-flex;align-items:center;gap:.5rem;min-height:2.6rem;padding:0 .72rem;border:1px solid " + (selected ? "var(--accent)" : "var(--border)") + ";border-radius:999px;background:" + (selected ? "var(--accent-soft)" : "var(--surface)") + ";color:" + (selected ? "var(--fg)" : "var(--fg-muted)") + ";cursor:pointer") }>
            <ChannelMark id={item.id}/>
            <span style={css("font-size:.71rem;font-weight:500")}>{item.label}</span>
            {selected && <Icon name="checkmark" size={12}/>} 
          </button>;
        })}
      </div>

      <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "minmax(0,1fr) minmax(16rem,.9fr)") + ";gap:1rem;margin-top:1.15rem;align-items:stretch") }>
        <div>
          <span style={css("display:block;font-size:.64rem;font-weight:500;letter-spacing:.05em;text-transform:uppercase;color:var(--fg-faint)")}>Duration</span>
          <div style={css("display:grid;grid-template-columns:repeat(3,1fr);max-width:26rem;margin-top:.55rem;border:1px solid var(--border);border-radius:999px;overflow:hidden;background:var(--surface)") }>
            {durations.map(item => <button key={item.weeks} type="button" aria-pressed={project.weeks === item.weeks} onClick={() => patch({ weeks: item.weeks })} style={css("min-height:2.35rem;border:none;border-right:" + (item.weeks !== 4 ? "1px solid var(--border-soft)" : "none") + ";background:" + (project.weeks === item.weeks ? "var(--accent)" : "transparent") + ";color:" + (project.weeks === item.weeks ? "#fff" : "var(--fg-muted)") + ";font-size:.7rem;font-weight:500;cursor:pointer")}>{item.label}</button>)}
          </div>

          <span style={css("display:block;margin-top:1rem;font-size:.64rem;font-weight:500;letter-spacing:.05em;text-transform:uppercase;color:var(--fg-faint)")}>Posts per week</span>
          <div style={css("display:flex;flex-direction:column;gap:.42rem;margin-top:.5rem") }>
            {project.channels.length ? project.channels.map(id => { const item = channel(id); return <div key={id} style={css("display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.7rem;align-items:center;min-height:2.3rem") }><div style={css("display:flex;align-items:center;gap:.5rem") }><ChannelMark id={id}/><span style={css("font-size:.73rem;font-weight:500")}>{item.label}</span></div><div style={css("display:flex;align-items:center;gap:.5rem") }><button type="button" aria-label={`Reduce ${item.label} cadence`} onClick={() => patch({ cadence: { ...project.cadence, [id]: Math.max(1, project.cadence[id] - 1) } })} style={css("width:1.8rem;height:1.8rem;border:1px solid var(--border);border-radius:.55rem;background:var(--surface);display:grid;place-items:center;cursor:pointer;color:var(--fg-muted)")}><Icon name="minus" size={11}/></button><strong style={css("min-width:1rem;text-align:center;font-size:.82rem;font-weight:500")}>{project.cadence[id]}</strong><button type="button" aria-label={`Increase ${item.label} cadence`} onClick={() => patch({ cadence: { ...project.cadence, [id]: Math.min(7, project.cadence[id] + 1) } })} style={css("width:1.8rem;height:1.8rem;border:1px solid var(--border);border-radius:.55rem;background:var(--surface);display:grid;place-items:center;cursor:pointer;color:var(--accent)")}><Icon name="plus" size={11}/></button></div></div>; }) : <p style={css("margin:.4rem 0 0;font-size:.7rem;color:var(--fg-faint)")}>Choose at least one channel to set the rhythm.</p>}
          </div>
        </div>
        ...
        <div style={css("display:flex;flex-direction:column;min-height:13rem;padding:1.1rem;border:1px solid var(--border-soft);border-radius:.95rem;background:var(--surface-alt)") }>
          <span style={css("font-size:.64rem;font-weight:500;letter-spacing:.05em;text-transform:uppercase;color:var(--fg-faint)")}>This plan</span>
          <div style={css("display:flex;align-items:baseline;gap:.45rem;margin-top:.45rem") }><strong style={css("font-size:2.15rem;line-height:1;font-weight:500")}>{total}</strong><span style={css("font-size:.8rem;color:var(--fg-muted)")}>posts</span></div>
          <p style={css("margin:.4rem 0 0;font-size:.75rem;color:var(--fg-muted)")}>across {project.channels.length} channel{project.channels.length === 1 ? "" : "s"} · {project.weeks} week{project.weeks === 1 ? "" : "s"}</p>
          <div style={css("display:flex;align-items:center;gap:.48rem;flex-wrap:wrap;margin-top:.72rem")}>{project.channels.map(id => { const item = channel(id); return <span key={id} style={css("display:inline-flex;align-items:center;gap:.34rem;font-size:.64rem;color:var(--fg-muted)")}><ChannelMark id={id} size={1.2} logoSize={10}/>{item.label}</span>; })}</div>
...
function BriefStage({ clientName, project, patch, analyze, buildPlan, mobile, aiBusy, aiError }: { clientName: string; project: SocialProject; patch: (next: Partial<SocialProject>) => void; analyze: () => void; buildPlan: () => void; mobile: boolean; aiBusy: "analyze" | "plan" | null; aiError: string }) {
...
function PlanStage({ project, patch, mobile, rebuild, aiBusy }: { project: SocialProject; patch: (next: Partial<SocialProject>) => void; mobile: boolean; rebuild: () => void; aiBusy: "analyze" | "plan" | null }) {
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.8rem") }>
    <Panel style="overflow:hidden">
      <div style={css("padding:1rem 1.05rem") }>
        <div style={css("padding:1rem;border:1px solid color-mix(in srgb,var(--accent) 28%,var(--border) 72%);border-radius:.95rem;background:var(--accent-soft)") }>
          <span style={css("font-size:.64rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--accent)")}>This month’s theme</span>
          <h2 style={css("margin:.35rem 0 0;font-size:1.28rem;font-weight:500;letter-spacing:-.01em")}>{theme.title}</h2>
          <p style={css("margin:.38rem 0 0;max-width:48rem;font-size:.77rem;line-height:1.55;color:var(--fg-muted)")}>{theme.description}</p>
        </div>

        <div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(7rem,1fr));gap:.55rem;margin-top:.75rem") }>
          {[{ label: "Posts", value: project.posts.length }, { label: "Cross-posts", value: crossPostCount }, { label: "Single channel", value: singleChannelCount }, { label: "Weeks", value: project.weeks }].map(item => <div key={item.label} style={css("padding:.8rem .9rem;border:1px solid var(--border-soft);border-radius:.85rem;background:var(--surface)")}><strong style={css("display:block;font-size:1.35rem;font-weight:500")}>{item.value}</strong><span style={css("display:block;margin-top:.2rem;font-size:.62rem;text-transform:uppercase;color:var(--fg-faint)")}>{item.label}</span></div>)}
        </div>

        <div style={css("margin-top:.9rem") }>
          <span style={css("font-size:.64rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--fg-faint)")}>Pillar mix</span>
          <div style={css("display:flex;flex-direction:column;gap:.58rem;margin-top:.55rem") }>{pillarCounts.map(item => { const percent = Math.round(item.count / Math.max(project.posts.length, 1) * 100); return <div key={item.pillar} style={css("display:grid;grid-template-columns:" + (mobile ? "minmax(7.5rem,.8fr) minmax(0,1fr) 3.4rem" : "minmax(9rem,.7fr) minmax(12rem,1.8fr) 4.2rem") + ";gap:.55rem;align-items:center") }><span style={css("display:flex;align-items:center;gap:.45rem;font-size:.7rem;font-weight:500") }><i style={css("width:.5rem;height:.5rem;border-radius:50%;background:" + item.color)}/>{item.pillar}</span><span style={css("height:.42rem;border-radius:999px;background:var(--border-soft);overflow:hidden") }><i style={css("display:block;height:100%;width:" + percent + "%;border-radius:999px;background:" + item.color)}/></span><span style={css("font-size:.66rem;color:var(--fg-muted);text-align:right")}>{item.count} · {percent}%</span></div>; })}</div>
        </div>

        <div style={css("margin-top:.9rem") }>
          <span style={css("font-size:.64rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--fg-faint)")}>Channel split</span>
          <div style={css("display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;margin-top:.55rem") }>{channelCounts.map(({ item, count }) => <span key={item.id} style={css("display:inline-flex;align-items:center;gap:.5rem;min-height:2.35rem;padding:0 .75rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);font-size:.71rem;font-weight:500") }><ChannelMark id={item.id} size={1.55} logoSize={13}/>{item.label}<small style={css("font-size:.64rem;font-weight:400;color:var(--fg-muted)")}>{count} posts</small></span>)}</div>
        </div>
      </div>
      <div style={css("display:flex;align-items:center;justify-content:flex-end;gap:.45rem;flex-wrap:wrap;padding:.8rem 1.05rem;border-top:1px solid var(--border-soft);background:var(--surface-alt)") }>
        ...
        <button type="button" disabled={aiBusy === "plan"} onClick={rebuild} style={css(buttonSoft + ";opacity:" + (aiBusy === "plan" ? ".5" : "1"))}><Icon name="replay" size={13}/>{aiBusy === "plan" ? "Re-drafting…" : "Re-draft"}</button>
    ...
    <Panel style="overflow:hidden">
      <div style={css("padding:.82rem 1rem;border-bottom:1px solid var(--border-soft);display:flex;align-items:center;justify-content:space-between;gap:.6rem") }><div><strong style={css("display:block;font-size:.8rem;font-weight:500")}>Planned content</strong><span style={css("font-size:.62rem;color:var(--fg-faint)")}>Choose one channel or cross-post an idea before mapping the month.</span></div><span style={css("font-size:.65rem;color:var(--accent)")}>{crossPostCount} cross-post{crossPostCount === 1 ? "" : "s"}</span></div>
      <div style={css("display:grid;grid-template-columns:repeat(auto-fill,minmax(13.5rem,1fr));gap:.55rem;padding:.7rem;max-height:23rem;overflow-y:auto")}>{project.posts.map(post => {
        ...
        return <article key={post.id} style={css("padding:.7rem;border:1px solid " + (crossPosted ? "color-mix(in srgb,var(--accent) 32%,var(--border-soft) 68%)" : "var(--border-soft)") + ";border-radius:.8rem;background:var(--surface-alt)")}>
          <div style={css("display:flex;align-items:center;justify-content:space-between;gap:.5rem") }><span style={css("display:inline-flex;align-items:center;gap:.38rem;font-size:.58rem;color:var(--fg-muted)")}><PostChannelMarks post={post}/>{postDestinationLabel(post)}</span><span style={css("font-size:.56rem;color:var(--fg-faint)")}>Day {post.day}</span></div>
          <strong style={css("display:block;margin-top:.4rem;font-size:.74rem;font-weight:500")}>{post.title}</strong>
          <p style={css("margin:.28rem 0 0;font-size:.64rem;line-height:1.42;color:var(--fg-muted);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden")}>{post.caption}</p>
          <div style={css("display:flex;align-items:center;justify-content:space-between;gap:.4rem;margin-top:.48rem") }><span style={css("font-size:.57rem;color:var(--fg-faint)")}>{post.pillar} · {post.format}</span><button type="button" disabled={project.channels.length < 2} onClick={() => toggleCrossPost(post)} style={css("min-height:1.7rem;padding:0 .48rem;border:1px solid " + (crossPosted ? "var(--accent)" : "var(--border)") + ";border-radius:999px;background:" + (crossPosted ? "var(--accent-soft)" : "var(--surface)") + ";color:" + (crossPosted ? "var(--accent)" : "var(--fg-muted)") + ";font-size:.56rem;cursor:" + (project.channels.length > 1 ? "pointer" : "default") + ";opacity:" + (project.channels.length > 1 ? "1" : ".45"))}>{crossPosted ? "Use one channel" : "+ Cross-post"}</button></div>
        </article>;
      })}</div>
    </Panel>
  </div>;
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.8rem") }>
    <Panel style="overflow:hidden">
      <div style={css("display:flex;align-items:center;justify-content:space-between;gap:.7rem;flex-wrap:wrap;padding:.85rem 1rem;border-bottom:1px solid var(--border-soft)")}>
        <div><span style={css("font-size:.62rem;text-transform:uppercase;letter-spacing:.04em;color:var(--accent)")}>Content calendar</span><h2 style={css("margin:.2rem 0 0;font-size:1.02rem;font-weight:500")}>{month}</h2></div>
        <div style={css("display:flex;gap:.4rem;align-items:center") }><span style={css("font-size:.64rem;color:var(--fg-muted)")}>{project.posts.filter(post => post.status === "approved").length}/{project.posts.length} approved</span><button type="button" onClick={approveAll} style={css(buttonSoft)}><Icon name="checkmark" size={13}/>Approve all</button></div>
      </div>
      <div data-social-calendar-viewport style={css("overflow-x:" + (mobile ? "auto" : "hidden") + ";padding:.6rem;box-sizing:border-box") }>
        <div data-social-calendar-grid style={css("width:100%;min-width:" + (mobile ? "39rem" : "0") + ";box-sizing:border-box") }>
          <div style={css("display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:.2rem;margin-bottom:.25rem")}>{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(day => <span key={day} style={css("font-size:.54rem;text-align:center;text-transform:uppercase;color:var(--fg-faint)")}>{day}</span>)}</div>
    ...
    {selectedPost && <Panel style="overflow:hidden;border-color:color-mix(in srgb,var(--accent) 34%,var(--border) 66%)">
      <div style={css("display:flex;align-items:center;gap:.7rem;flex-wrap:wrap;padding:.8rem 1rem;border-bottom:1px solid var(--border-soft)") }>
        ...
        <div><strong style={css("display:block;font-size:.86rem;font-weight:500")}>{postDestinationLabel(selectedPost)}</strong><span style={css("display:block;margin-top:.08rem;font-size:.66rem;color:var(--fg-muted)")}>{postChannelIds(selectedPost).map(id => channel(id).label).join(" + ")} · {month.split(" ")[0]} {selectedPost.day} · {selectedPost.time}</span></div>
        <span style={css("padding:.2rem .55rem;border-radius:999px;background:var(--accent-soft);color:var(--accent);font-size:.64rem")}>{selectedPost.format}</span>
        <div style={css("display:flex;align-items:center;gap:.38rem;margin-left:auto") }><span style={css("font-size:.68rem;color:var(--fg-faint);margin-right:.1rem")}>{selectedIndex + 1} / {project.posts.length}</span><button type="button" aria-label="Previous post" onClick={() => selectRelativePost(-1)} style={css("width:2rem;height:2rem;border:1px solid var(--border);border-radius:50%;background:var(--surface);color:var(--fg-muted);cursor:pointer")}>‹</button><button type="button" aria-label="Next post" onClick={() => selectRelativePost(1)} style={css("width:2rem;height:2rem;border:1px solid var(--border);border-radius:50%;background:var(--surface);color:var(--fg-muted);cursor:pointer")}>›</button></div>
      </div>

      <div style={css("display:grid;grid-template-columns:" + (mobile ? "1fr" : "minmax(12rem,.72fr) minmax(0,2.15fr)") + ";gap:1rem;padding:1rem") }>
        <div>
          <label style={css("display:grid;place-items:center;min-height:12rem;padding:1rem;border:1.5px dashed var(--border);border-radius:.9rem;background:var(--surface-alt);text-align:center;cursor:pointer") }>
            ...
            <span><Icon name="file" size={24}/><strong style={css("display:block;margin-top:.55rem;font-size:.78rem;font-weight:500")}>{selectedPost.assetName || `Drop ${selectedPost.format.toLowerCase()} art`}</strong><span style={css("display:block;margin-top:.25rem;font-size:.68rem;color:var(--fg-muted)")}>{selectedPost.assetName ? "Click to replace" : "or browse files"}</span><span style={css("display:block;margin-top:.65rem;font-size:.72rem;font-weight:500;color:var(--accent)")}>{selectedPost.graphicCopy || selectedPost.title}</span></span>
          </label>
          <div style={css("display:flex;align-items:center;justify-content:space-between;margin-top:.72rem") }><span style={css("font-size:.64rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--fg-faint)")}>Art format</span><span style={css("font-size:.62rem;color:var(--fg-faint)")}>{selectedPost.format === "Vertical video" || selectedPost.format === "Story" ? "1080 × 1920" : selectedPost.format === "Landscape image" ? "1920 × 1080" : "1080 × 1080"}</span></div>
          <div style={css("display:flex;flex-direction:column;gap:.38rem;margin-top:.48rem") }>{ART_FORMATS.map(format => { const active = selectedPost.format === format.value; return <button key={format.value} type="button" aria-pressed={active} onClick={() => updatePost(selectedPost.id, { format: format.value })} style={css("display:flex;align-items:center;gap:.55rem;min-height:2.35rem;padding:0 .68rem;border:1px solid " + (active ? "var(--accent)" : "var(--border-soft)") + ";border-radius:.7rem;background:" + (active ? "var(--accent-soft)" : "var(--surface)") + ";color:" + (active ? "var(--accent)" : "var(--fg-muted)") + ";font-size:.71rem;font-weight:500;cursor:pointer;text-align:left") }><span style={css("width:.85rem;height:" + (format.ratio === "9:16" ? "1.15rem" : format.ratio === "16:9" ? ".62rem" : ".85rem") + ";border:1.5px solid currentColor;border-radius:.12rem")}/>{format.label}<span style={css("margin-left:auto;font-size:.64rem;color:var(--fg-faint)")}>{format.ratio}</span></button>; })}</div>
        </div>
        ...
        <div>
          <div style={css("padding:.72rem .75rem;border:1px solid var(--border-soft);border-radius:.8rem;background:var(--surface-alt);margin-bottom:.8rem") }>
            <div style={css("display:flex;align-items:flex-start;justify-content:space-between;gap:.7rem;flex-wrap:wrap") }><div><span style={css("display:block;font-size:.64rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--fg-faint)")}>Publishing channels</span><span style={css("display:block;margin-top:.18rem;font-size:.63rem;color:var(--fg-muted)")}>Keep the primary channel selected, then add or remove cross-post destinations.</span></div><span style={css("padding:.2rem .5rem;border-radius:999px;background:" + (postChannelIds(selectedPost).length > 1 ? "var(--accent-soft)" : "var(--surface)") + ";border:1px solid " + (postChannelIds(selectedPost).length > 1 ? "color-mix(in srgb,var(--accent) 28%,var(--border) 72%)" : "var(--border)") + ";font-size:.6rem;color:" + (postChannelIds(selectedPost).length > 1 ? "var(--accent)" : "var(--fg-muted)"))}>{postDestinationLabel(selectedPost)}</span></div>
            <div style={css("display:flex;align-items:center;gap:.38rem;flex-wrap:wrap;margin-top:.58rem") }>{project.channels.map(id => {
              ...
              return <button key={id} type="button" aria-pressed={active} onClick={() => { if (primary) return; const current = selectedPost.crossPostTo || []; updatePost(selectedPost.id, { crossPostTo: current.includes(id) ? current.filter(item => item !== id) : [...current, id] }); }} style={css("display:inline-flex;align-items:center;gap:.36rem;min-height:2rem;padding:0 .58rem;border:1px solid " + (active ? "var(--accent)" : "var(--border)") + ";border-radius:999px;background:" + (active ? "var(--accent-soft)" : "var(--surface)") + ";color:" + (active ? "var(--fg)" : "var(--fg-muted)") + ";font-size:.62rem;cursor:" + (primary ? "default" : "pointer"))}><ChannelMark id={id} size={1.15} logoSize={9}/>{channel(id).label}{primary ? <small style={css("font-size:.52rem;color:var(--accent)")}>Primary</small> : active ? <Icon name="checkmark" size={10}/> : null}</button>;
            })}</div>
          </div>
          <input aria-label="Post title" value={selectedPost.title} onChange={event => updatePost(selectedPost.id, { title: event.target.value })} style={css("width:100%;box-sizing:border-box;border:none;background:transparent;color:var(--fg);padding:.05rem 0 .5rem;font:inherit;font-size:1.18rem;font-weight:500;outline:none")}/>
          <label style={css("display:block;margin-top:.35rem;font-size:.64rem;font-weight:500;text-transform:uppercase;letter-spacing:.05em;color:var(--fg-faint)")}>Caption</label>
          ...
          <label style={css("display:block;margin-top:.7rem;padding:.65rem .72rem;border-radius:.75rem;background:var(--surface-alt)") }><span style={css("font-size:.68rem;font-weight:500;color:var(--accent)")}>Art direction</span><textarea value={selectedPost.artDirection || `${selectedPost.format} with natural light and clear on-image copy.`} onChange={event => updatePost(selectedPost.id, { artDirection: event.target.value })} rows={2} style={css("width:100%;box-sizing:border-box;border:none;background:transparent;color:var(--fg-muted);padding:.25rem 0 0;font:inherit;font-size:.7rem;line-height:1.45;outline:none;resize:vertical")}/></label>
        </div>
      </div>
      ...
      <div style={css("display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;padding:.8rem 1rem;border-top:1px solid var(--border-soft);background:var(--surface-alt)") }>
        <button type="button" onClick={() => selectRelativePost(1)} style={css("border:none;background:none;color:var(--fg-faint);font-size:.72rem;cursor:pointer")}>Skip for now</button>
...
function ScheduleStage({ project, approved, exportCsv, actions, patch }: { project: SocialProject; approved: number; exportCsv: () => void; actions: PortalActions; patch: (next: Partial<SocialProject>) => void }) {
  ...
  return <div style={css("display:flex;flex-direction:column;gap:.8rem") }>
    <Panel style="padding:1.2rem">
      ...
      <span style={css("display:block;margin-top:.75rem;font-size:.65rem;text-transform:uppercase;letter-spacing:.04em;color:var(--success)")}>{project.sent ? "Scheduled" : "Calendar approved"}</span>
      <h2 style={css("margin:.28rem 0 0;font-size:1.18rem;font-weight:500")}>{project.sent ? "The month is queued and ready." : "The social plan is ready to schedule."}</h2>
      <p style={css("margin:.42rem 0 0;max-width:40rem;font-size:.75rem;line-height:1.55;color:var(--fg-muted)")}>Export the complete calendar for your scheduling tool or share this dashboard view for client review. Every caption, hashtag, date, and art format stays attached to its post.</p>
      <div style={css("display:grid;grid-template-columns:repeat(auto-fit,minmax(7rem,1fr));gap:.5rem;margin-top:.9rem")}>{[{ label: "Total posts", value: project.posts.length }, { label: "Approved", value: approved }, { label: "Cross-posts", value: crossPostCount }, { label: "Single channel", value: project.posts.length - crossPostCount }, { label: "Weeks", value: project.weeks }].map(item => <div key={item.label} style={css("padding:.72rem;border:1px solid var(--border-soft);border-radius:.75rem;background:var(--surface-alt)")}><strong style={css("display:block;font-size:1.1rem;font-weight:500;color:var(--accent)")}>{item.value}</strong><span style={css("display:block;margin-top:.18rem;font-size:.6rem;color:var(--fg-faint)")}>{item.label}</span></div>)}</div>
    ...
    <Panel style="overflow:hidden">
      <div style={css("overflow-x:auto") }>
        <div style={css("display:grid;grid-template-columns:4rem 8rem minmax(10rem,1fr) 7rem 6rem;gap:.6rem;padding:.65rem .8rem;border-bottom:1px solid var(--border-soft);background:var(--surface-alt);font-size:.58rem;text-transform:uppercase;color:var(--fg-faint);min-width:43.5rem") }><span>Date</span><span>Destinations</span><span>Content</span><span>Format</span><span>Status</span></div>
        <div style={css("max-height:26rem;overflow-y:auto;min-width:43.5rem")}>{project.posts.map(post => <div key={post.id} style={css("display:grid;grid-template-columns:4rem 8rem minmax(10rem,1fr) 7rem 6rem;gap:.6rem;align-items:center;padding:.65rem .8rem;border-bottom:1px solid var(--border-soft);font-size:.64rem") }><span style={css("color:var(--fg-faint)")}>Day {post.day}</span><span style={css("display:flex;align-items:center;gap:.38rem;color:" + (postChannelIds(post).length > 1 ? "var(--accent)" : "var(--fg-muted)"))}><PostChannelMarks post={post} size={1.25} logoSize={10}/><span>{postChannelIds(post).length > 1 ? "Cross-post" : "Single"}</span></span><span><strong style={css("display:block;font-size:.68rem;font-weight:500")}>{post.title}</strong><small style={css("display:block;margin-top:.12rem;font-size:.57rem;color:var(--fg-faint)")}>{post.time}</small></span><span style={css("color:var(--fg-muted)")}>{post.format}</span><span style={css("color:" + (post.status === "approved" ? "var(--success)" : "var(--fg-faint)"))}>{post.status}</span></div>)}</div>
      </div>
    </Panel>
  </div>;
}
