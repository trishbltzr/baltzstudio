...
export function QuickStart({ mode, accent, known, questionLabels, onApply, onContinue, showToast, mobile, clientName, clientId }: {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setScanStep(step => Math.min(step + 1, SCAN_STEPS.length - 1)), 1_150);
    return () => window.clearInterval(timer);
  }, [scanning]);

  const understand = async () => {
    if (scanning) return;
    if ((mode === "audit" || mode === "seo" || mode === "website_builder") && !link.trim()) { setScanError(mode === "website_builder" ? "Add the existing website URL for the revamp." : "Add the website URL you want to audit."); return; }
    ...
    if (mode === "funnel" && !link.trim() && !brief.trim()) { setScanError("Add a funnel URL or paste a brief."); return; }
    setScanning(true);
    setScanError("");
    try {
      const guidelineFiles = mode === "brand" ? await Promise.all(files.slice(0, 1).map(async file => {
        if (file.size > 3_000_000) throw new Error(`${file.name} is larger than 3 MB. Compress it before uploading.`);
      ...
      onApply(delta);
      showToast?.(`We found ${n} answer${n === 1 ? "" : "s"} across ${pages} page${pages === 1 ? "" : "s"} — review starts now`);
      onContinue?.();
    } catch (error) {
  ...
  const label = mode === "website_builder" ? "Add the existing site. We’ll map the revamp." : mode === "brand" ? "Add your brand sources. We’ll build the first pass." : mode === "seo" ? "Connect analytics and scan the website." : mode === "audit" ? "Drop the URL. We’ll do the first pass." : "AI Jumpstart this funnel";
  const sub = mode === "website_builder" ? "AI scans the current pages and sitemap to prefill the redesign intake. You’ll confirm what stays, changes, merges, or needs to be added." : mode === "brand" ? "Upload existing guidelines and add the website or public social profiles. AI will prefill the brand intake, then you’ll review every answer." : mode === "seo" ? "GA4 adds real behavioral evidence when connected. The website scan still works on its own when GA4 is unavailable." : mode === "audit"
    ...
    <div style={css("border:1px solid color-mix(in srgb," + accent + " 22%,var(--border-soft) 78%);border-radius:var(--radius-panel);background:color-mix(in srgb," + accent + " 5%,var(--surface) 95%);padding:" + (mobile ? "0.9rem 1rem" : "1.05rem 1.15rem"))}>
      ...
      <div style={css("display:flex;flex-direction:column;gap:0.55rem")}>
        <label style={css("display:flex;flex-direction:column;gap:0.32rem;font-size:0.8rem;font-weight:500;color:var(--fg-muted)")}>
          {mode === "website_builder" ? "Existing website URL" : mode === "brand" ? "Website URL (optional when guidelines are uploaded)" : mode === "seo" || mode === "audit" ? "Website URL" : "Link to an existing funnel (optional)"}
        ...
        {(mode === "brand" || mode === "audit" || mode === "funnel") && <div style={css("display:flex;flex-direction:column;gap:0.32rem;font-size:0.8rem;font-weight:500;color:var(--fg-muted)")}>
          {mode === "brand" || mode === "audit" ? "Brand guidelines (optional)" : "Working files (optional)"}
          <button type="button" onClick={() => fileRef.current?.click()} style={css("display:flex;align-items:center;gap:0.5rem;width:100%;box-sizing:border-box;border:1px dashed var(--border);border-radius:var(--radius);background:var(--surface);padding:0.55rem 0.65rem;cursor:pointer;text-align:left")}>
            ...
            <span style={css("flex:1;min-width:0;font-size:0.82rem;color:" + (files.length ? "var(--fg)" : "var(--fg-faint)"))}>{files.length ? files.map(file => file.name).join(", ") : (mode === "brand" || mode === "audit" ? "Upload brand-guidelines.pdf" : "Upload working files (Figma, docs, decks…)")}</span>
            {files.length > 0 && <span style={css("font-size:0.7rem;color:" + accent + ";font-weight:500")}>{files.length} file{files.length === 1 ? "" : "s"}</span>}
          </button>
          <input ref={fileRef} type="file" multiple={mode !== "brand"} hidden accept={mode === "brand" ? ".pdf,.txt,.md" : undefined} onChange={e => { const next = Array.from(e.target.files || []); if (next.length) setFiles(mode === "brand" ? next.slice(0, 1) : prev => [...prev, ...next]); e.currentTarget.value = ""; }} />
        </div>}
      ...
      <div style={css("display:flex;align-items:center;gap:0.6rem;margin-top:0.85rem;flex-wrap:wrap")}>
        <button type="button" onClick={() => void understand()} disabled={scanning} className="pt-op" style={css("display:inline-flex;align-items:center;gap:0.42rem;min-height:2.35rem;padding:0 1.05rem;border:none;border-radius:var(--radius-pill);background:" + accent + ";color:#fff;font-size:0.84rem;font-weight:500;cursor:" + (scanning ? "wait" : "pointer") + ";opacity:" + (scanning ? ".72" : "1"))}>
          <Icon name="sparkle" size={15} />{scanning ? "Scanning website…" : mode === "funnel" ? "AI understand & prefill" : `Scan & prefill ${remainingCount} questions`}
        </button>
      ...
      <p aria-live="polite" style={css("margin:0.55rem 0 0;font-size:0.78rem;line-height:1.45;color:var(--fg-muted)")}>{scanning ? SCAN_STEPS[scanStep] + "…" : "Next, you’ll review the prefilled Audit one question at a time."}</p>
    </div>
  );
}
