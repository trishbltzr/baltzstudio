  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(); }}>
      <div style={{ background: "var(--surface)", borderRadius: "var(--radius-panel)", border: "0", overflow: "hidden" }}>
        <div style={{ padding: "1.5rem 1.75rem 1rem", borderBottom: "1px solid var(--border-soft)" }}>
          <div style={{ fontSize: "var(--text-base)", color: "var(--fg-muted)", marginBottom: "0.65rem", display: "flex", alignItems: "center", gap: "0.45rem" }}>
          ...
          <h3 style={{ fontSize: "var(--text-2xl)", fontWeight: 500, color: "var(--fg)", lineHeight: 1.3, margin: 0 }}>{prompt.prompt}</h3>
          {prompt.label && <div style={{ fontSize: "var(--text-base)", color: "var(--fg-muted)", marginTop: "0.4rem" }}>{prompt.label} · {prompt.type}</div>}
        </div>
          ...
          {prompt.kind === "checklist" ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "0.45rem" }}>
              {prompt.options?.map(option => {
                ...
                return (
                  <label key={option} style={{ display: "flex", alignItems: "center", gap: "0.8rem", cursor: "pointer", padding: "0.65rem 0.9rem", borderRadius: "var(--radius)", border: `1px solid ${selected ? "oklch(0.78 0.11 22)" : "var(--border)"}`, background: selected ? "oklch(0.78 0.11 22 / 0.06)" : "transparent", transition: "all 0.15s" }}>
                    ...
                    <span style={{ fontSize: "var(--text-lg)", color: "var(--fg)", fontWeight: selected ? 600 : 400 }}>{option}</span>
                  </label>
                );
              })}
            </div>
      ...
      <div style={{ marginTop: "1rem", display: "flex", alignItems: "center", gap: "0.65rem" }}>
        ...
        {canUsePrimaryAction && (
          <span style={{ fontSize: "var(--text-base)", color: "var(--fg-muted)", display: "flex", alignItems: "center", gap: "0.3rem" }}>
      ...
      <label style={{ marginTop: "0.85rem", display: "flex", alignItems: "center", gap: "0.55rem", cursor: "pointer", userSelect: "none" }}>
